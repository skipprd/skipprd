use skippr_plugin_data_source_bing_webmaster_tools::*;
...
skippr_runtime_sdk::runtime_main!(async {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-bing-webmaster-tools: {}", err);
        std::process::exit(1);
    }
});
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "BingWebmasterTools".to_string(),
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-bing-webmaster-tools",
        "BingWebmasterTools",
        Some(capability),
        |start| {
            Box::pin(async move {
