...
#[test]
fn runtime_plugin_binaries_use_bounded_sdk_main() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let tokio_main = Regex::new(r"#\s*\[\s*tokio::main\b").unwrap();
    ...
    for relative_dir in [
            ...
            .filter(|entry| entry.file_name() == "main.rs")
        {
            ...
            if tokio_main.is_match(&contents) {
                violations.push(format!("{} uses #[tokio::main]", path.display()));
            }
            if !contents.contains("skippr_runtime_sdk::runtime_main!(") {
                violations.push(format!(
                    "{} must use skippr_runtime_sdk::runtime_main!",
                    path.display()
                ));
            }
        }
    ...
    assert!(main_count > 0, "no runtime plugin binaries were checked");
    assert!(
        violations.is_empty(),
        "runtime plugin binaries must use the bounded SDK main:\n{}",
        violations.join("\n")
    );
}
