...
impl DataSourceXAdsPlugin {
    pub fn new(config: DataSourceXAdsPluginConfig) -> Result<Self, std::io::Error> {
        config.validate()?;
    ...
    fn build_auth(
        ...
        if std::env::var("SKIPPR_X_ADS_FIXTURE_DIR")
            .map(|d| !d.trim().is_empty())
            .unwrap_or(false)
        {
            return Ok((Some(StaticBearerAuth::new("fixture")), None));
        }
        if let Ok(token) = std::env::var("X_ADS_ACCESS_TOKEN") {
        ...
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Meta Instagram Ads requires access_token, X_ADS_ACCESS_TOKEN, or \
...
mod tests {
    ...
    fn test_config() -> DataSourceXAdsPluginConfig {
        DataSourceXAdsPluginConfig {
    ...
    fn discover_mode_uses_minimal_streams_only() {
        let plugin = DataSourceXAdsPlugin::new(test_config()).unwrap();
    ...
    fn parses_campaign_adset_ad_and_placement_fixtures() {
        ...
        for (file, namespace) in [
            ("campaign_insights.json", "x_ads.campaign_daily"),
            ("adset_insights.json", "x_ads.adset_daily"),
            ("ad_insights.json", "x_ads.ad_daily"),
            (
                "campaign_placement_insights.json",
                "x_ads.campaign_placement_daily",
            ),
        ] {
    ...
    fn placement_contract_primary_key_includes_platform_fields() {
        let stream = streams_for_profile(StreamProfile::Full)
            ...
            .unwrap();
        let contract = DataSourceXAdsPlugin::namespace_contract(stream);
    ...
    fn loads_fixture_dir_for_account_rows() {
        ...
        let fixture_dir = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures");
        std::env::set_var("SKIPPR_X_ADS_FIXTURE_DIR", fixture_dir);
    ...
    fn config_defaults_and_validation_succeed() {
        ...
        DataSourceXAdsPlugin::new(cfg).expect("plugin init");
    }
    ...
    fn oauth_refresh_auth_accepted_when_fully_configured() {
        ...
        DataSourceXAdsPlugin::new(cfg).expect("oauth config accepted");
    }
    ...
    fn missing_credentials_returns_clear_error() {
        ...
        std::env::remove_var("SKIPPR_X_ADS_FIXTURE_DIR");
        std::env::remove_var("X_ADS_ACCESS_TOKEN");
        let err = match DataSourceXAdsPlugin::new(cfg) {
            Err(err) => err,
    ...
    fn invalid_start_date_rejected() {
        let err = DataSourceXAdsPlugin::parse_date("not-a-date").unwrap_err();
    ...
    fn end_date_before_start_date_rejected_on_sync() {
        ...
        let mut plugin = DataSourceXAdsPlugin::new(cfg).unwrap();
        let rt = tokio::runtime::Runtime::new().unwrap();
