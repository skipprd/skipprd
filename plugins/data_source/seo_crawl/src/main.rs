...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-seo-crawl: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-seo-crawl",
        "SeoCrawl",
        Some(capability),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("SeoCrawl")
                    .map_err(std::io::Error::other)?;
                let cfg: DataSourceSeoCrawlPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourceSeoCrawlPlugin::new(cfg)?;
