...
impl Buffers {
    ...
    fn spawn_s3_pair_reclaim(
        ...
        tokio::spawn(async move {
            ...
            if let Err(err) =
                crate::buffer::segment_object::SegmentObject::reclaim_pair(&client, &bucket, &key)
                    .await
            {
...
async fn wal_recover_s3(offsets_db: Arc<Offsets>) -> io::Result<()> {
    ...
    for seg_key in segs.iter() {
        ...
        let commit_bytes =
            match crate::helpers::s3::get_object_bytes_for_bucket(&bucket, &commit_key).await {
                Ok(bytes) => bytes,
                Err(e) => {
                    warn!("S3 WAL recovery: commit GET {}: {}", commit_key, e);
                    continue;
                }
            };
        match client
            ...
            .await
        {
            Ok(resp) => match resp.body.collect().await {
                Ok(agg) => {
                    ...
                    let meta = match SegmentFile::admit_owned_pair_bytes(&data, &commit_bytes) {
                        Ok(m) => m,
                        Err(e) => {
                            warn!(
                                "S3 WAL recovery: refusing unbindable pair {}: {}",
                                seg_key, e
                            );
                            continue;
                        }
