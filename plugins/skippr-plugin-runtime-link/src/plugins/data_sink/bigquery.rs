...
use crate::helpers::configuration::{Config, DataSinkBigqueryPluginConfig};
...
impl DataSinkBigqueryPlugin {
    ...
    fn load_config() -> DataSinkBigqueryPluginConfig {
        DataSinkBigqueryPluginConfig {
            ...
            location: {
                let v = Config::getenv("BIGQUERY_LOCATION", "");
                if v.is_empty() {
                    None
                } else {
                    Some(v)
            ...
            credentials_path: {
                let v = Config::getenv("GOOGLE_APPLICATION_CREDENTIALS", "");
                if v.is_empty() {
                    None
                } else {
                    Some(v)
    ...
    fn resolve_credentials_path(&self) -> Result<String, String> {
        if let Some(ref p) = self.config.credentials_path {
            if !p.is_empty() {
                return Ok(p.clone());
            }
        }
        std::env::var("GOOGLE_APPLICATION_CREDENTIALS")
            .map_err(|_| {
                "BigQuery requires credentials_path or GOOGLE_APPLICATION_CREDENTIALS".to_string()
            })
            .and_then(|v| {
                if v.trim().is_empty() {
                    Err("GOOGLE_APPLICATION_CREDENTIALS is empty".to_string())
                } else {
                    Ok(v)
                }
            })
...
impl DataSink for DataSinkBigqueryPlugin {
    ...
    fn capability(&self) -> Option<&'static crate::plugins::cdc::SinkCapability> {
        Some(&crate::plugins::cdc::sink_capabilities::BIGQUERY)
    }
}
