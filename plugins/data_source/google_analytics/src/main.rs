use skippr_plugin_data_source_google_analytics::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-google-analytics: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "GoogleAnalytics".to_string(),
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-google-analytics",
        "GoogleAnalytics",
        Some(capability),
        |start| {
            Box::pin(async move {
