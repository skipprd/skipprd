...
// This tool is used for explicit approval flows (e.g., MetricFlow/DBT artifact approvals).
...
impl Tool for AskApprovalTool {
	fn name(&self) -> &'static str { "ask_approval" }
	async fn call(&self, args: Value, _ctx: &AgentCtx) -> Result<Value, String> {
		let prompt = args.get("prompt").and_then(|x| x.as_str()).unwrap_or("Please review and approve/reject:").to_string();
