...
use skippr_plugin_runtime_link::runtime_sink_link::gcs::{DataSinkGcsPlugin, DataSinkGcsPluginConfig};
...
async fn run() -> io::Result<()> {
    ...
    let handshake = match read_host_frame(&mut stdin).await? {
        ...
        other => {
            ...
            return Err(io::Error::other(
                "runtime gcs sink did not receive a handshake",
            ));
        }
    };
    ...
    let response = HandshakeResponse {
        ...
        plugin_name: "Gcs".to_string(),
        source_capability: None,
        sink_capability: cdc::sink_capabilities::by_name("Gcs").map(Into::into),
...
{
    let mut primary_plugin: Option<DataSinkGcsPlugin> = None;
    let mut deadletter_plugin: Option<DataSinkGcsPlugin> = None;
...
async fn run_sink_request(
    ...
    primary_plugin: &mut Option<DataSinkGcsPlugin>,
    deadletter_plugin: &mut Option<DataSinkGcsPlugin>,
) -> io::Result<()> {
    request
        .config
        .0
