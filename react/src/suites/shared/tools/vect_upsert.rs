...
use crate::vector::lance_store::Chunk;
...
impl Tool for VectUpsertTool {
    ...
    async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
        // Expect args: [{kind, dataset_id, field?, text, meta}]
		...
		for (i, v) in arr.iter().enumerate() {
            ...
            let dataset_id = v.get("dataset_id").and_then(|x| x.as_str()).unwrap_or("").to_string();
            ...
            items.push(Chunk { id, kind, dataset_id, field, text, vector: vecs.get(i).cloned().unwrap_or_default(), meta, epoch });
        }
