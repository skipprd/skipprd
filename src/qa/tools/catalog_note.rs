...
impl Tool for CatalogNoteTool {
	fn name(&self) -> &'static str { "catalog_note" }
	async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
		let pipeline = args.get("pipeline").and_then(|x| x.as_str()).unwrap_or("").to_string();
		if pipeline.is_empty() { return Err("pipeline required".to_string()); }
		let namespace_opt = args.get("namespace").and_then(|x| x.as_str()).map(|s| s.to_string());
		...
		if text.is_empty() { return Err("text required".to_string()); }
		let tags: Vec<String> = args.get("tags").and_then(|x| x.as_array()).map(|a| a.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect()).unwrap_or_default();
		let preview = args.get("preview").and_then(|x| x.as_bool()).unwrap_or(false);
		let thread_id = ctx.thread_id.clone().unwrap_or_default();
		// Resolve catalog key
		let (catalog_key, mut catalog): (String, Value) = if let Some(ns) = namespace_opt.as_ref() {
			let entry = crate::sql::registry::find_entry(&pipeline, ns).await.ok_or_else(|| "catalog entry not found".to_string())?;
			if entry.catalog_key.is_empty() { return Err("catalog_key missing for namespace".to_string()); }
			let mut v = crate::helpers::s3::get_json(&entry.catalog_key).await.map_err(|e| format!("get_json: {:?}", e))?;
			(entry.catalog_key, v)
		} else {
			// dataset-less: attach to a pipeline-level catalog if exists; else error
			return Err("namespace required for catalog_note".to_string());
		};
		// Extract existing facts
		let mut existing_facts: Vec<String> = Vec::new();
		let mut target_level = "dataset".to_string();
		if let Some(ns) = namespace_opt.as_ref() {
			if let Some(fld) = field_opt.as_ref() {
				target_level = "field".to_string();
				if let Some(fields) = catalog.get_mut("fields").and_then(|x| x.as_array()) {
					for f in fields {
						if f.get("name").and_then(|x| x.as_str()) == Some(fld.as_str()) {
							if let Some(gn) = f.get("governance_notes").and_then(|g| g.get("facts")).and_then(|x| x.as_array()) {
								for it in gn { if let Some(s) = it.as_str() { existing_facts.push(s.to_string()); } }
							}
							break;
						}
					}
				...
				if let Some(gn) = catalog.get("governance_notes").and_then(|g| g.get("dataset")).and_then(|x| x.get("facts")).and_then(|x| x.as_array()) {
					for it in gn { if let Some(s) = it.as_str() { existing_facts.push(s.to_string()); } }
				}
			}
		}
		...
		for line in bullets_text.lines() {
			...
			if !t.is_empty() { proposed.push(t.to_string()); }
			if proposed.len() >= 8 { break; }
		}
		...
		for (i, cand) in proposed.iter().enumerate() {
			...
			if let Some(vv) = v {
				for ev in existing_vecs.iter() {
					let c = cosine(vv, ev);
					if c >= 0.92 { dup = true; break; }
				}
			}
			if !dup { curated.push(cand.clone()); }
			if curated.len() >= 8 { break; }
		}
		...
		if let Some(ns) = namespace_opt.as_ref() {
			if let Some(fld) = field_opt.as_ref() {
				...
				let gn_parent = catalog.as_object_mut().unwrap().entry("governance_notes").or_insert(serde_json::json!({ "dataset": { "facts": [], "digest": "", "provenance": [], "last_updated": "" }}));
				let ds = gn_parent.get_mut("dataset").unwrap();
				merge_notes(ds, &curated, &digest, &prov, &now);
			}
		}
		// Write back atomically
		crate::helpers::s3::put_json(&catalog_key, &catalog).await.map_err(|e| format!("put_json: {:?}", e))?;
		...
		if let Ok(vv) = llm.embed(&[digest.clone()]) {
			if let Some(v) = vv.get(0) { vec1 = v.clone(); }
		}
		if !vec1.is_empty() {
			let id = format!("doc:{}:{}:catalog_note", pipeline, namespace_opt.clone().unwrap_or_default());
			let chunk = crate::qa::vector::lance_store::Chunk {
				id,
				kind: "doc".to_string(),
				namespace: namespace_opt.clone().unwrap_or_default(),
...
fn merge_notes(obj: &mut Value, curated: &[String], digest: &str, provenance: &Value, now: &str) {
	...
	// provenance log
	let prov = o.entry("provenance").or_insert(serde_json::json!([])).as_array_mut().unwrap();
