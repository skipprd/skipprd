...
impl DataSourceAdRollAdsPlugin {
    ...
    fn build_auth(
        ...
        Err(std::io::Error::new(
            std::io::ErrorKind::InvalidInput,
            "Meta Instagram Ads requires access_token, ADROLL_ADS_ACCESS_TOKEN, or \
...
mod tests {
    ...
    fn test_config() -> DataSourceAdRollAdsPluginConfig {
        DataSourceAdRollAdsPluginConfig {
    ...
    fn discover_mode_uses_minimal_streams_only() {
        let plugin = DataSourceAdRollAdsPlugin::new(test_config()).unwrap();
    ...
    fn parses_campaign_adset_ad_and_placement_fixtures() {
        ...
        for (file, namespace) in [
            ("campaign_insights.json", "adroll_ads.campaign_daily"),
            ("adset_insights.json", "adroll_ads.adset_daily"),
            ("ad_insights.json", "adroll_ads.ad_daily"),
            (
                "campaign_placement_insights.json",
                "adroll_ads.campaign_placement_daily",
            ),
        ] {
    ...
    fn placement_contract_primary_key_includes_platform_fields() {
        let stream = streams_for_profile(StreamProfile::Full)
            ...
            .unwrap();
        let contract = DataSourceAdRollAdsPlugin::namespace_contract(stream);
    ...
    fn config_defaults_and_validation_succeed() {
        ...
        DataSourceAdRollAdsPlugin::new(cfg).expect("plugin init");
    }
    ...
    fn oauth_refresh_auth_accepted_when_fully_configured() {
        ...
        DataSourceAdRollAdsPlugin::new(cfg).expect("oauth config accepted");
    }
    ...
    fn missing_credentials_returns_clear_error() {
        ...
        std::env::remove_var("SKIPPR_ADROLL_ADS_FIXTURE_DIR");
        std::env::remove_var("ADROLL_ADS_ACCESS_TOKEN");
        let err = match DataSourceAdRollAdsPlugin::new(cfg) {
            Err(err) => err,
    ...
    fn invalid_start_date_rejected() {
        let err = DataSourceAdRollAdsPlugin::parse_date("not-a-date").unwrap_err();
    ...
    fn end_date_before_start_date_rejected_on_sync() {
        ...
        let mut plugin = DataSourceAdRollAdsPlugin::new(cfg).unwrap();
        let rt = tokio::runtime::Runtime::new().unwrap();
