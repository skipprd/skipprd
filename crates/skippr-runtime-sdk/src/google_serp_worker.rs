//! Resolve the Google SERP ranks Node worker script path for local dev and packaged runs.

use std::collections::HashSet;
use std::path::{Path, PathBuf};

pub const WORKER_SCRIPT_FILE: &str = "google-serp-worker.mjs";
pub const REPO_WORKER_REL_PATH: &str =
    "plugins/data_source/google_serp_ranks/worker/google-serp-worker.mjs";

/// Optional override via `SKIPPR_GOOGLE_SERP_WORKER_SCRIPT`, then search:
...
pub fn resolve_google_serp_worker_script(
    plugin_crate_root: Option<&Path>,
) -> Result<PathBuf, std::io::Error> {
    if let Ok(path) = std::env::var("SKIPPR_GOOGLE_SERP_WORKER_SCRIPT") {
        ...
        if !trimmed.is_empty() {
            ...
            return Err(std::io::Error::new(
                ...
                format!(
                    "SKIPPR_GOOGLE_SERP_WORKER_SCRIPT points to missing file: {}",
                    path.display()
                ),
            ));
        }
    ...
    Err(std::io::Error::new(
        std::io::ErrorKind::NotFound,
        format!(
            "google serp worker script not found (expected {REPO_WORKER_REL_PATH} in a skipprd checkout, or set SKIPPR_GOOGLE_SERP_WORKER_SCRIPT)"
        ),
    ))
}
...
fn push_repo_worker_candidates(base: &Path, push: &mut dyn FnMut(PathBuf)) {
    ...
    if base.file_name().is_some_and(|name| name == "google_serp_ranks") {
...
mod tests {
    ...
    fn resolves_from_plugin_crate_root() {
        let _guard = env_lock();
        std::env::remove_var("SKIPPR_GOOGLE_SERP_WORKER_SCRIPT");
        ...
        let resolved =
            resolve_google_serp_worker_script(Some(temp.path())).expect("resolve worker");
