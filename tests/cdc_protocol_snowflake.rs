...
///
/// Run: `cargo test --test cdc_protocol_snowflake -- --ignored`
...
impl SnowflakeTestClient {
    ...
    fn authenticate_keypair() -> Result<String, String> {
        ...
        let account_upper = SNOWFLAKE_ACCOUNT.to_uppercase();
        let user_upper = SNOWFLAKE_USER.to_uppercase();
...
fn fq_tombstone(table: &str) -> String {
    tombstone_table_name(table)
}

// -----------------------------------------------------------------------
// 1. DDL: order token column + tombstone table creation
// -----------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn cdc_snowflake_ddl_order_token_and_tombstone() {
    ...
    let tombstone = fq_tombstone(&table);
    ...
    let ddl = ddl_add_order_token_column(SqlDialect::Snowflake, &table);
    sf.execute_sql(&ddl).await.unwrap();

    // Idempotent — running again must not fail
    sf.execute_sql(&ddl).await.unwrap();

    let tombstone_ddl = ddl_create_tombstone_table(
...
// -----------------------------------------------------------------------
// 2. Upsert-if-newer: insert then replay with stale token is rejected
// -----------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn cdc_snowflake_upsert_if_newer() {
    ...
    // Insert with order_token = "0000000000000002"
    ...
    // Newer update: order_token = "0000000000000005"
    ...
    // Stale replay: order_token = "0000000000000002" must NOT overwrite
    ...
    let name = sf
        ...
        .await
        .unwrap();
    assert_eq!(name, "bob", "stale replay must not overwrite newer row");
...
// -----------------------------------------------------------------------
// 3. Delete-if-newer: tombstone blocks stale resurrect
// -----------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn cdc_snowflake_delete_if_newer() {
    ...
    // Delete with newer token
    let del = delete_if_newer_sql(
        SqlDialect::Snowflake,
        &table,
        &tombstone,
        &[r#""ID""#.into()],
    ...
    // Stale insert must not resurrect (tombstone has order_token 0020)
    ...
    sf.execute_multi_sql(&sql_stale).await.unwrap();
    ...
    assert_eq!(
        count, "0",
        "stale insert must not resurrect a tombstoned row"
    );
...
// -----------------------------------------------------------------------
// 4. Replay idempotency: same upsert applied twice yields same result
// -----------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn cdc_snowflake_replay_idempotency() {
    ...
    let count = sf
        ...
        .await
        .unwrap();
    assert_eq!(count, "1", "replay should not duplicate rows");

    let name = sf
        .query_scalar(&format!(r#"SELECT "NAME" FROM {} WHERE "ID" = 42"#, table))
        .await
        .unwrap();
    assert_eq!(name, "replay_test");
...
// -----------------------------------------------------------------------
// 5. Composite key: multi-column business key works correctly
// -----------------------------------------------------------------------
#[tokio::test]
#[ignore]
async fn cdc_snowflake_composite_key() {
    ...
    // Insert (1, 200) — different composite key, same tenant
    let sql2 = upsert_if_newer_sql(
        SqlDialect::Snowflake,
        &table,
        &tombstone,
    ...
    let count = sf
        ...
        .await
        .unwrap();
    assert_eq!(count, "2", "two distinct composite keys should exist");

    // Update (1, 100) with newer token
    let sql3 = upsert_if_newer_sql(
        SqlDialect::Snowflake,
        &table,
        &tombstone,
    ...
    let email = sf
        .query_scalar(&format!(
            r#"SELECT "EMAIL" FROM {} WHERE "TENANT_ID" = 1 AND "USER_ID" = 100"#,
            table
        ))
        .await
        .unwrap();
    assert_eq!(email, "updated@b.com");

    // Other row untouched
    let email2 = sf
        .query_scalar(&format!(
            r#"SELECT "EMAIL" FROM {} WHERE "TENANT_ID" = 1 AND "USER_ID" = 200"#,
            table
        ))
        .await
        .unwrap();
    assert_eq!(email2, "c@d.com");
