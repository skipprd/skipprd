use skippr_plugin_data_source_content_quality::*;
...
skippr_runtime_sdk::runtime_main!(async {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-content-quality: {}", err);
        std::process::exit(1);
    }
});
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-content-quality",
        "ContentQuality",
        Some(capability),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("ContentQuality")
                    .map_err(std::io::Error::other)?;
                let cfg: DataSourceContentQualityPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourceContentQualityPlugin::new(cfg)?;
