use skippr_plugin_data_source_google_serp_ranks::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-google-serp-ranks: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "GoogleSerpRanks".to_string(),
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-google-serp-ranks",
        "GoogleSerpRanks",
        Some(capability),
        |start| {
            Box::pin(async move {
