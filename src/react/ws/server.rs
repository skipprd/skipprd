...
use crate::react::agent::{Agent, AgentCtx, RunOutcome};
// Removed unused tool imports; flows handle registry/tool selection
use uuid::Uuid;
use crate::react::ws::api_gen as api;
use std::collections::{HashMap, VecDeque};
use chrono::Utc;
use crate::models as m;
use crate::react::session::ThreadStore;
use crate::react::suites::registry::SuiteRegistry;
use crate::react::suites::SuiteCtx;
use std::sync::Arc;
...
pub async fn start_with_ctx(port: u16, suite_ctx: SuiteCtx) -> Result<(), String> {
    let reg = Arc::new(crate::react::suites::registry::default_registry());
...
async fn handle_message(text: &str, state: &mut ConnState) -> Result<Vec<String>, String> {
	...
	match typ {
		...
		"new" => {
			...
			{
				let store = state.thread_store();
				let _ = store.append_step(&thread_id, crate::react::session::ThreadStep {
		...
		"delete" => {
			...
			if let Some(vs) = state.suite_ctx.vector.as_ref() {
                let _ = vs.delete_thread_embeddings(&state.suite_ctx.scope.project_id, &thread_id).await;
			}
			// respond ok
			let mut ok = api::OkResponse::new(1, m::ok_response::Type::Ok, now_iso());
			ok.cid = Some(cid);
...
async fn synthesize_title(llm: &crate::react::providers::DynLlmProvider, question: &str, answer: &str) -> String {
...
async fn run_agent_and_frames(
	...
	// Delegate to suites
    let convert = |ff: crate::react::flow_frame::FlowFrame| -> AgentFrame {
		match ff {
			crate::react::flow_frame::FlowFrame::Final { answer, sql } => AgentFrame::Final { answer, sql },
			crate::react::flow_frame::FlowFrame::AwaitUser { prompt } => AgentFrame::AwaitUser { prompt },
			crate::react::flow_frame::FlowFrame::AwaitApproval { prompt } => AgentFrame::AwaitApproval { prompt },
		}
	};
	let suite = reg
		.get(suite_id)
...
fn compute_unread_for_log(log: &crate::react::session::ThreadLog, seen_seq: i32) -> (i32, i32) {
	let mut tseq: i32 = 0;
...
async fn build_history(store: &crate::react::session::ThreadStore, thread_id: &str, before: Option<i32>, limit_opt: Option<i32>) -> (Vec<api::HistoryResponseMessagesInner>, Option<i32>) {
