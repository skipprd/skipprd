...
impl DataSourceDynamodbPlugin {
    ...
    async fn consume_shard(
        ...
        let stored_seq = offsets.load_checkpoint(&shard_ckpt_key);
        ...
        if let Some(ref seq_bytes) = stored_seq {
            ...
            iter_builder = iter_builder
                ...
                .sequence_number(seq_str.as_ref());
        } else {
            ...
            if records.is_empty() {
                ...
                if !batch.is_empty() {
                    ...
                    if let Some(ref seq) = last_seq {
                        offsets.store_checkpoint(&shard_ckpt_key, seq.as_bytes());
                    }
                }
            }
