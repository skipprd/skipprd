...
- `clustered` — local disk WAL plus synchronous peer replication (two durable copies), a clock-free lease, and DynamoDB or Cloud Tables offsets/checkpoints. Reuses `SKIPPR_OFFSET_DYNAMODB_TABLE`. Requires a release binary built with `--features offset-store-dynamodb,offset-store-cloud-tables`. `sync --once` and `discover` are rejected; `sync` runs the long-lived scheduler; `query` is query-only and never takes an ingest lease. Automatic failover with continued writes needs three live processes.
