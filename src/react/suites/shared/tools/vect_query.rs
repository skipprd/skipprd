...
impl Tool for VectQueryTool {
    ...
    async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
        ...
        let mut all_hits: Vec<crate::react::vector::lance_store::ScoredChunk> = Vec::new();
        let mut errors: Vec<(String, String)> = Vec::new();
        let pipelines = vec![ctx.scope.project_id.clone()];
        for p in pipelines {
            match vector.query(&p, &vec, k, scope_str).await {
        ...
        let mut dedup: Vec<crate::react::vector::lance_store::ScoredChunk> = Vec::new();
        for h in all_hits {
        ...
        if dedup.is_empty() {
            ...
            if let Some("dataset") = scope_str {
                ...
                let mut all_hits2: Vec<crate::react::vector::lance_store::ScoredChunk> = Vec::new();
                let pipelines2 = vec![ctx.scope.project_id.clone()];
                for p in pipelines2 {
                    if let Ok(mut v) = vector.query(&p, &vec, k, None).await { all_hits2.append(&mut v); }
                }
                ...
                let mut dedup2: Vec<crate::react::vector::lance_store::ScoredChunk> = Vec::new();
                for h in all_hits2 {
