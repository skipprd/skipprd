...
pub async fn open_lease_store(table: String) -> Result<Arc<dyn PipelineLeaseStore>, String> {
    let kind = crate::pipeline_backend::configured_kind()?;
...
pub async fn open_membership_store(
    ...
    match crate::pipeline_backend::configured_kind()? {
        ...
        OffsetStoreKind::Sled => {
            Err("clustered membership requires SKIPPR_OFFSET_STORE=dynamodb or cloud-tables".into())
        }
    }
}
...
pub fn uses_cloud_tables() -> bool {
    matches!(
        crate::pipeline_backend::configured_kind(),
