...
impl FieldStats {
    ...
    pub fn finalize(&mut self) {
        ...
        // Histogram for numeric fields (opt-in via env to keep core dependency-free)
        if stats_histogram_enabled() {
...
impl DatasetFieldStats {
    pub fn new(dataset_id: &str) -> Self {
        Self { dataset_id: dataset_id.to_string(), fields: HashMap::new(), last_updated_epoch_ms: current_millis() }
    }
