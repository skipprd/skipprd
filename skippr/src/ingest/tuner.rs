...
pub fn apply_env_caps() {
	...
	if let Ok(v) = Config::getenv("UPLOAD_CONCURRENCY", "").parse::<usize>() { if v > 0 {
		...
		if Config::log_wal_enabled() { info!("tune: upload_concurrency set by env={}", v); }
	}}
	...
	if let Ok(v) = Config::getenv("WAL_COMPACTION_CONCURRENCY", "").parse::<usize>() { if v > 0 {
		...
		if Config::log_wal_enabled() { info!("tune: wal_compaction set by env={}", v); }
	}}
	...
	if let Ok(v) = Config::getenv("S3_DOWNLOAD_CONCURRENCY", "").parse::<usize>() { if v > 0 {
		...
		if Config::log_wal_enabled() { info!("tune: s3_download set by env={} (clamped)", clamped); }
	}}
}
...
pub fn tick(active: usize, capacity: usize, queued: usize, pressure: f64) {
	...
	// Error-aware tuning: compute EMA of S3 WAL retry rate and throttle if above threshold
	{
		...
		let ema = ((prev.saturating_mul(80)).saturating_add(delta.saturating_mul(100).saturating_mul(20))) / 100;
		...
		if ema > 200 {
			...
			let wc = crate::metrics::counters::WAL_COMPACTION_CONCURRENCY_TARGET.load(Ordering::Relaxed);
			let new_uc = uc.saturating_sub(1).max(2);
			let new_wc = wc.saturating_sub(1).max(2);
			if new_uc != uc {
				crate::metrics::counters::UPLOAD_CONCURRENCY_TARGET.store(new_uc, Ordering::Relaxed);
				if Config::log_wal_enabled() { debug!("tune: upload_concurrency {} -> {} (ema_wal_retry_x100={})", uc, new_uc, ema); }
			}
			if new_wc != wc {
				crate::metrics::counters::WAL_COMPACTION_CONCURRENCY_TARGET.store(new_wc, Ordering::Relaxed);
				if Config::log_wal_enabled() { debug!("tune: wal_compaction {} -> {} (ema_wal_retry_x100={})", wc, new_wc, ema); }
			}
		} else if ema < 50 {
...
pub fn compute_optimal_chunk_size(inputs: &ChunkSizeInputs) -> usize {
	...
	let occupancy = if inputs.max_queue_length == 0 { 0.0 } else { (inputs.queue_length as f32) / (inputs.max_queue_length as f32) };
	...
	if next < optimal_chunk_size_min { next = optimal_chunk_size_min; }
	...
	if let Some(avail_mib) = inputs.avail_mem_mib_opt {
		...
		if next > dyn_cap { next = dyn_cap; }
	} else {
		if next > inputs.max_chunk_size { next = inputs.max_chunk_size; }
	}
	next
}
...
pub fn tune_chunk_size(
	...
	while throughput_history.len() > 10 { throughput_history.pop_front(); }
	// Compute trend
	let trend = if throughput_history.len() < 2 {
		...
		if time_diff == 0.0 { 0.0 } else { (newest.1 as f64 - oldest.1 as f64) / time_diff }
	};
	// Compute next size
