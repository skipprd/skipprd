use skippr_plugin_data_source_dataforseo_backlinks::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-dataforseo-backlinks: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "DataForSeoBacklinks".to_string(),
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-dataforseo-backlinks",
        "DataForSeoBacklinks",
        Some(capability),
        |start| {
            Box::pin(async move {
