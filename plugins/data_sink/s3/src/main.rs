...
use skippr_plugin_runtime_link::runtime_sink_link::s3::{DataSinkS3Plugin, DataSinkS3PluginConfig};
...
async fn run() -> io::Result<()> {
    ...
    let handshake = match read_host_frame(&mut stdin).await? {
        ...
        other => {
            ...
            return Err(io::Error::other(
                "runtime s3 sink did not receive a handshake",
            ));
        }
    };
    ...
    let response = HandshakeResponse {
        ...
        sink_capability: cdc::sink_capabilities::by_name("S3").map(Into::into),
...
{
    let mut primary_plugin: Option<DataSinkS3Plugin> = None;
    let mut deadletter_plugin: Option<DataSinkS3Plugin> = None;
...
async fn run_sink_request(
    ...
    primary_plugin: &mut Option<DataSinkS3Plugin>,
    deadletter_plugin: &mut Option<DataSinkS3Plugin>,
) -> io::Result<()> {
    request
        .config
        .0
    ...
    if plugin_slot.is_none() {
        let config: DataSinkS3PluginConfig = request.config.0.decode().map_err(io::Error::other)?;
        *plugin_slot = Some(
