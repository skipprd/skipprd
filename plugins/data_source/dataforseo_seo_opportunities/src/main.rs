use skippr_plugin_data_source_dataforseo_seo_opportunities::*;
...
skippr_runtime_sdk::runtime_main!(async {
    if let Err(err) = run().await {
        eprintln!(
            "skippr-plugin-data-source-dataforseo-seo-opportunities: {}",
            err
        );
        std::process::exit(1);
    }
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "DataForSeoSeoOpportunities".to_string(),
