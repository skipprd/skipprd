...
impl From<DataSinkPluginConfig> for DataSinkRedshiftPluginConfig {
    fn from(plugin_config: DataSinkPluginConfig) -> Self {
        match plugin_config {
            DataSinkPluginConfig::Redshift(config) => config,
            _ => panic!("Invalid plugin type for Redshift"),
        }
    }
}
...
impl DataSinkRedshiftPlugin {
    ...
    async fn sync_via_s3(
        ...
        let s3_key = format!("{}/{}.parquet", prefix.trim_matches('/'), hex::encode(&md5_digest.0));
        ...
        info!("Redshift: staged {} rows to s3://{}/{}", row_count, bucket, s3_key);

        let iam_role = self
            .config
            .iam_role_arn
            .as_deref()
            .ok_or_else(|| std::io::Error::other("Redshift: iam_role_arn required for S3 COPY"))?;
        ...
        info!("Redshift: COPY INTO {} complete ({} rows)", table_name, row_count);
