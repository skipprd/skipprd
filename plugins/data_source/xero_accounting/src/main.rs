use skippr_plugin_data_source_xero_accounting::*;
...
skippr_runtime_sdk::runtime_main!(async {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-xero-accounting: {}", err);
        std::process::exit(1);
    }
});
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-xero-accounting",
        "XeroAccounting",
        Some(capability),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("XeroAccounting")
                    .map_err(std::io::Error::other)?;
                let cfg: DataSourceXeroAccountingPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourceXeroAccountingPlugin::new(cfg)?;
