...
fn is_deadletter_athena_target(config: &DataSinkAthenaPluginConfig, namespace: &str) -> bool {
    ...
    match Config::get_pipeline_deadletter_plugin_config() {
        Ok(Some(entry)) => {
            if entry.plugin_name != "Athena" {
                return false;
            }
            match entry.decode_for_plugin::<DataSinkAthenaPluginConfig>("Athena") {
                Ok(deadletter_config) => deadletter_config == *config,
                Err(_) => false,
            }
        }
        _ => false,
...
impl DataSinkAthenaPlugin {
    ...
    pub fn get_config() -> DataSinkAthenaPluginConfig {
        let fallback = || DataSinkAthenaPluginConfig {
            format: None,
        ...
        match Config::get_pipeline_output_plugin_config() {
            Ok(entry) => entry
                .decode_for_plugin::<DataSinkAthenaPluginConfig>("Athena")
