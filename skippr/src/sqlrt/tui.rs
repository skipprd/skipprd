...
fn build_bar_data(batches: &Vec<RecordBatch>) -> (Vec<String>, Vec<f64>) {
    if batches.is_empty() { return (Vec::new(), Vec::new()); }
    let b = &batches[0];
    if b.num_columns() == 0 { return (Vec::new(), Vec::new()); }
    // Try to use first two columns: label(string-ish) + value(numeric)
    let mut labels: Vec<String> = Vec::new();
    let mut values: Vec<f64> = Vec::new();
    let cols = b.num_columns();
    for r in 0..b.num_rows() {
        let label = if matches!(b.column(0).data_type(), ArrowDataType::Utf8) { value_to_string(b.column(0).as_ref(), r) } else { r.to_string() };
        let mut val: Option<f64> = None;
        if cols > 1 {
            match b.column(1).data_type() {
                ArrowDataType::Int64 => { if let Some(v) = b.column(1).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>().map(|a| a.value(r) as f64) { val = Some(v); } }
                ArrowDataType::Float64 => { if let Some(v) = b.column(1).as_any().downcast_ref::<datafusion::arrow::array::Float64Array>().map(|a| a.value(r)) { val = Some(v); } }
                _ => {}
            }
        }
        labels.push(label);
        values.push(val.unwrap_or(0.0));
        if labels.len() >= 50 { break; }
    }
    (labels, values)
}

fn build_histogram_data(batches: &Vec<RecordBatch>, bins: usize) -> Vec<(f64, f64)> {
    if batches.is_empty() { return Vec::new(); }
    let b = &batches[0];
    if b.num_columns() == 0 { return Vec::new(); }
    // Use first numeric column
    let mut vals: Vec<f64> = Vec::new();
    for c in 0..b.num_columns() {
        match b.column(c).data_type() {
            ArrowDataType::Int64 => { let a = b.column(c).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>().unwrap(); for r in 0..a.len() { if !a.is_null(r) { vals.push(a.value(r) as f64); } } break; }
            ArrowDataType::Float64 => { let a = b.column(c).as_any().downcast_ref::<datafusion::arrow::array::Float64Array>().unwrap(); for r in 0..a.len() { if !a.is_null(r) { vals.push(a.value(r)); } } break; }
            _ => {}
        }
    }
    ...
    for v in vals { let mut idx = ((v - min) / width).floor() as isize; if idx < 0 { idx = 0; } if idx as usize >= bins { idx = bins as isize - 1; } counts[idx as usize] += 1; }
    let mut out: Vec<(f64, f64)> = Vec::new();
    for i in 0..bins { out.push((min + i as f64 * width, counts[i] as f64)); }
    out
}
...
fn build_timeseries_data(batches: &Vec<RecordBatch>, max_points: usize) -> Vec<(f64, f64)> {
    if batches.is_empty() { return Vec::new(); }
    let b = &batches[0];
    if b.num_columns() < 2 { return Vec::new(); }
    ...
    for c in 0..b.num_columns() {
        match b.column(c).data_type() {
            ArrowDataType::Timestamp(_, _) => { if ts_col.is_none() { ts_col = Some(c); } }
            ArrowDataType::Int64 | ArrowDataType::Float64 => { if val_col.is_none() { val_col = Some(c); } }
            _ => {}
        }
    }
    let (tc, vc) = match (ts_col, val_col) { (Some(t), Some(v)) => (t, v), _ => return Vec::new() };
    let mut pts: Vec<(f64, f64)> = Vec::new();
    for r in 0..b.num_rows() {
        // TimestampMillisecondArray is the common case
        let t_ms = b.column(tc).as_any().downcast_ref::<datafusion::arrow::array::TimestampMillisecondArray>().map(|a| a.value(r)).unwrap_or(0) as f64;
        let v = match b.column(vc).data_type() {
            ArrowDataType::Int64 => b.column(vc).as_any().downcast_ref::<datafusion::arrow::array::Int64Array>().map(|a| a.value(r) as f64).unwrap_or(0.0),
            ArrowDataType::Float64 => b.column(vc).as_any().downcast_ref::<datafusion::arrow::array::Float64Array>().map(|a| a.value(r)).unwrap_or(0.0),
            _ => 0.0
        };
        pts.push((t_ms, v));
...
enum ViewMode { Table, BarV, BarH, Histogram, TimeSeries }

fn view_modes_labels() -> Vec<&'static str> { vec!["Table", "BarV", "BarH", "Hist", "Series"] }
fn mode_to_index(m: ViewMode) -> usize { match m { ViewMode::Table => 0, ViewMode::BarV => 1, ViewMode::BarH => 2, ViewMode::Histogram => 3, ViewMode::TimeSeries => 4 } }
fn index_to_mode(i: usize) -> ViewMode { match i { 0 => ViewMode::Table, 1 => ViewMode::BarV, 2 => ViewMode::BarH, 3 => ViewMode::Histogram, 4 => ViewMode::TimeSeries, _ => ViewMode::Table } }

impl QueryEditorView {
    pub fn new(initial: &str) -> Self { Self { input: initial.to_string(), cursor: initial.len(), rows_offset: 0, sugg_idx: 0, mode: ViewMode::Table, menu_active: false, menu_idx: 0 } }

    pub fn run(self, cfg: QueryEditorConfig, result_rx: mpsc::Receiver<Vec<RecordBatch>>, request_tx: mpsc::Sender<String>) {
        let mut s = self;
        ...
        loop {
            ...
            if let Ok(b) = result_rx.try_recv() { data = b; s.rows_offset = 0; }
            ...
            let schema_arc_opt = if !data.is_empty() { Some(data[0].schema()) } else { None };
            ...
            if s.sugg_idx >= suggestions.len() { s.sugg_idx = 0; }

            let _ = terminal.draw(|f| {
                let chunks = Layout::default()
                    .direction(Direction::Vertical)
                    .constraints([Constraint::Length(1), Constraint::Min(1), Constraint::Length(3), Constraint::Length(1)].as_ref())
                    .split(f.size());
                ...
                match s.mode {
                    ViewMode::Table => {
                        let table = if headers.is_empty() {
                            ...
                            let body_rows_iter = rows.iter().skip(s.rows_offset).map(|r| Row::new(r.iter().map(|c| Cell::from(c.clone()))));
                    ...
                    ViewMode::BarV | ViewMode::BarH => {
                        ...
                        for (i, v) in values.iter().enumerate() { if i < owned.len() { items.push((owned[i].as_str(), (*v) as u64)); } }
                        let mut bar = BarChart::default().block(Block::default().borders(Borders::ALL).title("bar"))
                            .data(&items);
                        if s.mode == ViewMode::BarH { bar = bar.direction(Direction::Horizontal); }
                        f.render_widget(bar, chunks[1]);
                    }
                    ViewMode::Histogram => {
                        let pts = build_histogram_data(&data, 20);
                        let dataset = Dataset::default().name("hist").marker(ratatui::symbols::Marker::Braille)
                            .graph_type(GraphType::Line)
                            .style(Style::default().fg(Color::Cyan))
                            .data(&pts);
                        let x_bounds = if pts.is_empty() { [0.0, 1.0] } else { [pts.first().unwrap().0, pts.last().unwrap().0] };
                    ...
                    ViewMode::TimeSeries => {
                        ...
                        let dataset = Dataset::default().name("ts").graph_type(GraphType::Line).style(Style::default().fg(Color::Green)).data(&pts);
                        let x_bounds = if pts.is_empty() { [0.0, 1.0] } else { [pts.first().unwrap().0, pts.last().unwrap().0] };
                        let y_max = pts.iter().map(|p| p.1).fold(0.0, f64::max).max(1.0);
                        let chart = Chart::new(vec![dataset])
                ...
                if let Some(sg) = suggestions.get(s.sugg_idx) {
                    if !tok.is_empty() {
                        ...
                        if at_word_end && !equals_suggestion {
                            ...
                            if candidate.to_lowercase().starts_with(&tok.to_lowercase()) && candidate.len() > tok.len() {
                                let suffix = &candidate[tok.len()..];
                                segs.push(Span::styled(suffix.to_string(), Style::default().fg(Color::DarkGray)));
                            }
                        }
                    }
                }
                ...
                if s.menu_active {
                    ...
                    for (i, label) in modes.iter().enumerate() {
                        if i == s.menu_idx { items.push(format!("[{}]", label)); } else { items.push(label.to_string()); }
                    }
                    ...
                    if !suggestions.is_empty() {
                        let max = 5usize;
                        let start = if s.sugg_idx >= max { s.sugg_idx - (max - 1) } else { 0 };
                        let end = std::cmp::min(start + max, suggestions.len());
                        let mut items: Vec<String> = Vec::new();
                        for (i, sg) in suggestions.iter().enumerate().take(end).skip(start) {
                            if i == s.sugg_idx { items.push(format!("[{}]", sg)); } else { items.push(sg.clone()); }
                        }
                        if !foot.is_empty() { foot.push_str(" | "); }
            ...
            if event::poll(std::time::Duration::from_millis(100)).unwrap_or(false) {
                match event::read().unwrap() {
                    Event::Key(k) => match k.code {
                        KeyCode::Esc => { if s.menu_active { s.menu_active = false; } else { break; } }
                        // Command+R (SUPER+R on mac) to run; Enter also runs
                        KeyCode::Char('r') if k.modifiers.contains(KeyModifiers::SUPER) => { let _ = request_tx.send(s.input.clone()); }
                        KeyCode::Enter => { let _ = request_tx.send(s.input.clone()); }
                        // Open menu (F2)
                        KeyCode::F(2) => {
                            s.menu_active = true;
                        ...
                        // Close menu (F10)
                        KeyCode::F(10) => { s.menu_active = false; }
                        // Menu navigation
                        KeyCode::Up if s.menu_active => {
                            let modes_len = view_modes_labels().len();
                            if modes_len > 0 { s.menu_idx = (s.menu_idx + modes_len - 1) % modes_len; s.mode = index_to_mode(s.menu_idx); }
                        }
                        KeyCode::Down if s.menu_active => {
                            let modes_len = view_modes_labels().len();
                            if modes_len > 0 { s.menu_idx = (s.menu_idx + 1) % modes_len; s.mode = index_to_mode(s.menu_idx); }
                        }
                        // When menu is open, ignore other input edits
                        key if s.menu_active => { let _ = key; }
                        // Option+Tab (Alt+Tab) to move to next word; Shift+Alt+Tab to previous word
                        KeyCode::Tab if k.modifiers.contains(KeyModifiers::ALT) && k.modifiers.contains(KeyModifiers::SHIFT) => { s.cursor = prev_word(&s.input, s.cursor); }
                        KeyCode::Tab if k.modifiers.contains(KeyModifiers::ALT) => { s.cursor = next_word(&s.input, s.cursor); }
                        ...
                        KeyCode::Up if k.modifiers.contains(KeyModifiers::ALT) => { if s.sugg_idx > 0 { s.sugg_idx -= 1; } }
                        KeyCode::Down if k.modifiers.contains(KeyModifiers::ALT) => { if !suggestions.is_empty() { s.sugg_idx = (s.sugg_idx + 1) % suggestions.len(); } }
                    // Ctrl+N / Ctrl+P to cycle suggestions (like readline/zsh)
                    KeyCode::Char('n') if k.modifiers.contains(KeyModifiers::CONTROL) => { if !suggestions.is_empty() { s.sugg_idx = (s.sugg_idx + 1) % suggestions.len(); } }
                    KeyCode::Char('p') if k.modifiers.contains(KeyModifiers::CONTROL) => { if s.sugg_idx > 0 { s.sugg_idx -= 1; } }
                        // View mode toggles (legacy direct bindings still supported with Option)
                        KeyCode::Char('t') if k.modifiers.contains(KeyModifiers::ALT) => { s.mode = ViewMode::Table; }
                        KeyCode::Char('v') if k.modifiers.contains(KeyModifiers::ALT) => { s.mode = ViewMode::BarV; }
                        KeyCode::Char('h') if k.modifiers.contains(KeyModifiers::ALT) => { s.mode = ViewMode::BarH; }
                        KeyCode::Char('y') if k.modifiers.contains(KeyModifiers::ALT) => { s.mode = ViewMode::Histogram; }
                        KeyCode::Char('s') if k.modifiers.contains(KeyModifiers::ALT) => { s.mode = ViewMode::TimeSeries; }
                        KeyCode::Left => { if s.cursor > 0 { s.cursor = prev_char_boundary(&s.input, s.cursor); } }
                        KeyCode::Right => { if s.cursor < s.input.len() { s.cursor = next_char_boundary(&s.input, s.cursor); } }
                        ...
                        KeyCode::Up => { s.rows_offset = s.rows_offset.saturating_sub(1); }
                        KeyCode::Down => { s.rows_offset = s.rows_offset.saturating_add(1); }
                        KeyCode::PageUp => { s.rows_offset = s.rows_offset.saturating_sub(20); }
                        KeyCode::PageDown => { s.rows_offset = s.rows_offset.saturating_add(20); }
                        KeyCode::Char(c) => { s.input.insert(s.cursor, c); s.cursor = s.cursor.saturating_add(c.len_utf8()); }
                        _ => {}
                    },
                    _ => {}
                }
...
fn input_cursor_pos(area: Rect, cursor: usize) -> (u16, u16) {
    ...
    (area.x.saturating_add(1 + (cursor as u16).saturating_sub(x_offset)), area.y.saturating_add(1))
}
...
fn s_len_chars(s: &str) -> usize { s.chars().count() }

fn next_word(input: &str, cursor: usize) -> usize {
    let mut i = cursor;
    while i < input.len() { let nb = next_char_boundary(input, i); let ch = input[i..nb].chars().next().unwrap_or('\0'); i = nb; if is_word_char(ch) { break; } }
    while i < input.len() { let nb = next_char_boundary(input, i); let ch = input[i..nb].chars().next().unwrap_or('\0'); if !is_word_char(ch) { break; } i = nb; }
    i
}
...
fn prev_word(input: &str, cursor: usize) -> usize {
    ...
    let mut i = cursor;
    while i > 0 { let pb = prev_char_boundary(input, i); let ch = input[pb..i].chars().next().unwrap_or('\0'); i = pb; if is_word_char(ch) { break; } }
    while i > 0 { let pb = prev_char_boundary(input, i); let ch = input[pb..i].chars().next().unwrap_or('\0'); if !is_word_char(ch) { break; } i = pb; }
    i
}

fn current_token<'a>(input: &'a str, cursor: usize) -> (&'a str, usize) {
    let mut start = cursor;
    while start > 0 { let cb = prev_char_boundary(input, start); let c = input[cb..start].chars().next().unwrap_or('\0'); if is_word_char(c) { start = cb; } else { break; } }
    (&input[start..cursor], start)
}

fn build_suggestions(input: &str, prefix: &str, current_schema: Option<&ArrowSchema>, cursor: usize, batches: &Vec<RecordBatch>) -> Vec<String> {
    ...
    let in_where_value_ctx = in_where_ctx && is_where_value_position(input, cursor, where_pos.unwrap());
    ...
    if after_from_ctx {
        kw.extend(["where", "group", "order", "limit", "join", "left", "inner", "right", "full"].iter().copied());
    } else if in_where_ctx {
        kw.extend(["and", "or", "not", "group", "order", "limit"].iter().copied());
        ...
        kw.extend(["select", "from", "where", "group", "order", "limit", "stream", "window", "and", "or", "not", "as"].iter().copied());
    }
    let udfs = ["lateness", "new_session", "zscore", "is_outlier_z", "to_timestamp_millis"];
    for k in kw.iter() { if !has_prefix || k.starts_with(&prefix.to_lowercase()) { out.push(k.to_string()); } }
    // UDFs relevant mainly in projection/where/order/group contexts
    if is_projection_ctx || in_where_ctx || in_order_ctx || in_group_ctx {
        for u in udfs.iter() { if u.starts_with(&prefix.to_lowercase()) { out.push(u.to_string()); } }
    }
    ...
    if after_from_ctx || (!is_projection_ctx && !in_where_ctx && !in_order_ctx && !in_group_ctx) {
        ...
        for t in Config::get_pipelines() { if !has_prefix || t.starts_with(prefix) { out.push(t); } }
    }
    // LIMIT numeric suggestions
    if in_limit_ctx {
        let nums = ["10", "100", "1000", "5000", "10000"]; // common limits
        for n in nums.iter() { if !has_prefix || n.starts_with(prefix) { out.push(n.to_string()); } }
    }
    ...
    if is_projection_ctx || (in_where_ctx && !in_where_value_ctx) || in_order_ctx || in_group_ctx {
        if let Some(table) = table_opt {
            ...
            } else if let Some(schema) = current_schema { // fallback: use current result schema
                let mut fields: Vec<String> = Vec::new();
        ...
        } else if let Some(schema) = current_schema { // no table parsed yet; still suggest from current schema
            let mut fields: Vec<String> = Vec::new();
            flatten_fields(schema, None, &mut fields);
            for f in fields { if !has_prefix || f.starts_with(prefix) { out.push(f); } }
        }
    }
    ...
    if in_where_value_ctx {
        if let Some((table_opt2, alias_opt2)) = extract_table_and_alias(input).into() {
            if let Some(lhs_field) = extract_lhs_field_in_where(input, cursor, where_pos.unwrap_or(0)) {
                let target = match alias_opt2.as_deref() {
                    Some(alias) if lhs_field.starts_with(&format!("{}.", alias)) => lhs_field[alias.len() + 1..].to_string(),
                    _ => lhs_field.clone()
                };
                if let Some(b) = batches.get(0) {
                    let schema = b.schema();
                    if let Some((col_idx, _)) = schema.fields().iter().enumerate().find(|(_, f)| f.name() == &target) {
...
fn is_where_value_position(input: &str, cursor: usize, where_pos_idx: usize) -> bool {
    ...
    let ops = [" = ", " != ", " <> ", " >= ", " <= ", " > ", " < ", " IN ", " LIKE ", " BETWEEN "];
    let mut last_op: Option<usize> = None;
    for op in ops.iter() { if let Some(idx) = segment.rfind(op) { last_op = Some(last_op.map_or(idx, |cur| cur.max(idx))); } }
    if last_op.is_none() { return false; }
    ...
    for b in bounds.iter() { if let Some(idx) = segment.rfind(b) { last_bound = Some(last_bound.map_or(idx, |cur| cur.max(idx))); } }
    match last_bound { Some(b) => last_op_pos > b, None => true }
}
...
fn extract_lhs_field_in_where(input: &str, cursor: usize, where_pos_idx: usize) -> Option<String> {
    ...
    for op in ops.iter() {
        if let Some(idx) = seg_low.rfind(op) { last = Some(last.map_or(idx, |cur| cur.max(idx))); }
    }
    let pos = last?;
    ...
    // skip spaces
    while i > 0 && bytes[i].is_ascii_whitespace() { i = i.saturating_sub(1); }
    // collect word chars (., _, alnum)
    let mut start = i;
    while start > 0 {
        let ch = bytes[start] as char;
        if ch.is_alphanumeric() || ch == '_' || ch == '.' { start = start.saturating_sub(1); } else { break; }
    }
...
fn flatten_field(field: &ArrowField, parent: Option<&str>, out: &mut Vec<String>) {
    ...
    match field.data_type() {
        ArrowDataType::Struct(fields) => {
            out.push(name.clone());
            for ch in fields { flatten_field(ch, Some(&name), out); }
        }
...
fn extract_table_name(sql: &str) -> Option<String> {
    ...
    if let Some(i) = up.find(" FROM ") {
        ...
        for ch in rest.chars() { if ch.is_alphanumeric() || ch == '_' || ch == '-' { name.push(ch); } else { break; } }
...
fn extract_table_and_alias(sql: &str) -> (Option<String>, Option<String>) {
    ...
    if let Some(i) = up.find(" FROM ") {
        ...
        if let Some(table_tok) = it.next() {
            // strip punctuation
            let table_name: String = table_tok.chars().take_while(|ch| ch.is_alphanumeric() || *ch == '_' || *ch == '-').collect();
            if table_name.is_empty() { return (None, None); }
            let mut alias_opt: Option<String> = None;
            if let Some(next_tok) = it.next() {
                let next_up = next_tok.to_uppercase();
                if next_up == "AS" {
                    if let Some(alias_tok) = it.next() { alias_opt = Some(alias_tok.chars().take_while(|ch| ch.is_alphanumeric() || *ch == '_').collect()); }
                } else {
                    // treat immediate token as alias if not a keyword
                    let is_kw = matches!(next_up.as_str(), "WHERE" | "GROUP" | "ORDER" | "LIMIT" | "JOIN" | "INNER" | "LEFT" | "RIGHT" | "FULL" | "ON");
                    if !is_kw { alias_opt = Some(next_tok.chars().take_while(|ch| ch.is_alphanumeric() || *ch == '_').collect()); }
                }
            }
...
fn is_in_select_list(input: &str, cursor: usize) -> bool {
    ...
    if let Some(sel_idx) = up.rfind("SELECT") {
        ...
        if after_cursor.contains(" FROM ") || up[sel_idx..].contains(" FROM ") == false {
            ...
            if i > 0 && bytes[i-1] as char == '*' { return false; }
            return true;
        }
    }
    false
