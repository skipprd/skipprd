...
pub struct DataSourceMetaAdsPluginConfig {
...
pub struct DataSourceMetaAdsPlugin {
...
impl DataSourceMetaAdsPlugin {
    pub fn new(config: DataSourceMetaAdsPluginConfig) -> Result<Self, std::io::Error> {
        config.validate()?;
    ...
    fn build_auth(
        ...
        if let Ok(token) = std::env::var("META_ADS_ACCESS_TOKEN") {
...
impl DataSourceMetaAdsPluginConfig {
...
#[async_trait]
impl DataSource for DataSourceMetaAdsPlugin {
...
mod tests {
    ...
    fn test_config() -> DataSourceMetaAdsPluginConfig {
        DataSourceMetaAdsPluginConfig {
    ...
    fn parses_campaign_adset_ad_and_placement_fixtures() {
        ...
        for (file, namespace) in [
            ("campaign_insights.json", "meta_ads.campaign_daily"),
            ("adset_insights.json", "meta_ads.adset_daily"),
            ("ad_insights.json", "meta_ads.ad_daily"),
            (
                "campaign_placement_insights.json",
                "meta_ads.campaign_placement_daily",
            ),
        ] {
            let body: serde_json::Value = serde_json::from_str(
    ...
    fn placement_contract_primary_key_includes_platform_fields() {
        let stream = streams_for_profile(StreamProfile::Full)
            ...
            .unwrap();
        let contract = DataSourceMetaAdsPlugin::namespace_contract(stream);
    ...
    fn config_defaults_and_validation_succeed() {
        ...
        DataSourceMetaAdsPlugin::new(cfg).expect("plugin init");
    }
    ...
    fn oauth_refresh_auth_accepted_when_fully_configured() {
        ...
        DataSourceMetaAdsPlugin::new(cfg).expect("oauth config accepted");
    }
    ...
    fn missing_credentials_returns_clear_error() {
        ...
        std::env::remove_var("SKIPPR_META_ADS_FIXTURE_DIR");
        std::env::remove_var("META_ADS_ACCESS_TOKEN");
        let err = match DataSourceMetaAdsPlugin::new(cfg) {
            Err(err) => err,
    ...
    fn invalid_start_date_rejected() {
        let err = DataSourceMetaAdsPlugin::parse_date("not-a-date").unwrap_err();
