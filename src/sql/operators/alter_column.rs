...
pub fn alter_column_type(metadata: &mut Metadata, alteration: &AlterTableAlterColumnType) -> Result<Metadata, String> {
    ...
    let skippr_new_type = SkipprTypes::from_string(&alteration.new_type.to_string())
        .ok_or_else(|| format!("No type equivalent for '{}' in Skippr types", alteration.new_type))?;

    match skippr_new_type {
        SkipprTypes::Array => {
            let values_new_type = alteration.values_new_type.clone()
            ...
            let skippr_value_type = SkipprTypes::from_string(&values_new_type.to_string())
                .ok_or_else(|| format!("No type equivalent for '{}' in Skippr types", values_new_type))?;
