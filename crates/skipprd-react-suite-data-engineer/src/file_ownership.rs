...
//! | ---------------- | -------------------------------------------------------------------- | ------------------------------------- | ---------------------------------------------------------------------------------------------------- |
//! | `SkipprOwned`    | `profiles.yml`, `packages.yml`, `macros/generate_schema_name.sql`    | LLM cannot write                      | System-scoped, authoritative, immutable to the agent. Connection identity, dependency governance, and provider-owned namespace routing. |
...
//! | `Generated`      | `models/schema.yml`, `models/staging/*.yml`                         | LLM cannot write directly; deterministic schema-contract tools author these files | Column contracts are compiler output from SQL/truth, not free-form patches. |
//! | `LlmOwned`       | `models/**/*.sql`, `seeds/*`, `macros/*.sql`                        | LLM can write                         | Modeling logic and reusable Jinja are the agent's primary surface.                                             |
...
pub enum Ownership {
    ...
    LlmOwned,
    /// Deterministically generated source file. The agent may request generation through a tool,
    /// but cannot free-form patch it through the file tool.
...
const SKIPPR_OWNED_FILES: &[SkipprOwnedFile] = &[
    ...
    SkipprOwnedFile {
        path_or_prefix: "macros/generate_schema_name.sql",
        reason: "`generate_schema_name` is provider-owned namespace routing. The agent cannot author this macro.",
        regenerated_each_run: true,
    },
];
...
const GENERATED_SCHEMA_REASON: &str =
    "Schema YAML is generated from SQL output columns and project truth. Use the schema contract generation tool instead of patching this file directly.";
...
pub fn classify_rel_path(rel: &str) -> Ownership {
    ...
    if rel == "models/schema.yml"
        || (rel.starts_with("models/staging/") && (rel.ends_with(".yml") || rel.ends_with(".yaml")))
    {
        return Ownership::Generated {
            reason: GENERATED_SCHEMA_REASON,
        };
    }
...
pub fn is_writable_by_llm(rel: &str) -> bool {
    match classify_rel_path(rel) {
        Ownership::SkipprOwned { .. } | Ownership::BuildArtifact | Ownership::Generated { .. } => {
            false
        }
...
pub fn deny_list_hint(rel: &str) -> Option<&'static str> {
    match classify_rel_path(rel) {
        ...
        Ownership::Generated { reason } => Some(reason),
...
mod tests {
    ...
    fn file_ownership_classifies_known_paths() {
        ...
        assert!(matches!(
            classify_rel_path("macros/generate_schema_name.sql"),
            Ownership::SkipprOwned { .. }
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            classify_rel_path("models/staging/stg_orders.yml"),
            Ownership::Generated { .. }
        ));
        assert!(matches!(
    ...
    fn is_writable_by_llm_matches_ownership_class() {
        ...
        assert!(!is_writable_by_llm("models/staging/stg_orders.yml"));
        assert!(!is_writable_by_llm("macros/generate_schema_name.sql"));
    ...
    #[test]
    fn deny_list_hint_returns_generated_schema_text() {
        let hint = deny_list_hint("models/schema.yml").expect("hint");
        assert!(hint.contains("Schema YAML is generated"));
        assert!(hint.contains("schema contract generation tool"));
    }
