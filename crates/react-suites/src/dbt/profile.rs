...
pub fn generate_profiles_yml(cfg: &ReactResolvedConfig) -> Result<GeneratedProfiles, String> {
    ...
    match active {
        ActiveWarehouse::Athena => {
            ...
            let region_value = if ath.region.trim().is_empty() {
                "{{ env_var('AWS_REGION', env_var('AWS_DEFAULT_REGION', 'us-east-1')) }}".to_string()
            } else {
                ath.region.trim().to_string()
            };
            let s3_staging_dir = ath.result_s3.trim().to_string();
            if s3_staging_dir.is_empty() {
                return Err(
                    "providers.athena.result_s3 is required to generate DBT athena profile".to_string(),
                );
            }
