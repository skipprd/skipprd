...
struct ThreadView {
    ...
    // Plan snapshots keyed by suite-defined `planKind`.
    plans_by_kind: HashMap<String, api::PlanSnapshot>,
    // Sticky phase anchors for each plan kind (usually "<kind>_plan").
    plan_anchor_phase_by_kind: HashMap<String, String>,
...
impl Default for ThreadView {
    fn default() -> Self {
        Self {
            ...
            items: BTreeMap::new(),
            plans_by_kind: HashMap::new(),
            plan_anchor_phase_by_kind: HashMap::new(),
...
fn apply_event(m: &mut Model, ev: TerminalEvent) {
    match ev {
        ...
        TerminalEvent::Plans { thread_id, plans } => {
            let tv = m
                .threads
            ...
            for p in plans {
                ...
                tv.plans_by_kind.insert(kind.clone(), p);
                tv.plan_anchor_phase_by_kind
                    .entry(kind.clone())
                    .or_insert_with(|| format!("{kind}_plan"));
            }
            tv.last_update = Instant::now();
        ...
        TerminalEvent::PlansChanged(ev) => {
            ...
            if let Some(keys) = ev.changed_plan_keys.as_ref() {
                for (idx, kind) in ev.changed.iter().enumerate() {
                    let Some(new_key) = keys.get(idx) else {
                        continue;
                    };
                    if tv
                        .plans_by_kind
...
fn llm_label_for_phase(phase: &str) -> String {
    ...
    for (idx, part) in p.split('_').enumerate() {
        if idx > 0 {
            friendly.push(' ');
        }
        let mut chars = part.chars();
        if let Some(first) = chars.next() {
            friendly.push(first.to_ascii_uppercase());
            friendly.push_str(chars.as_str());
        }
    }
    format!("LLM · {friendly}")
}
...
fn render_thread_detail(t: &ThreadView, spinner_idx: usize) -> Vec<String> {
    ...
    let mut anchor_by_kind: HashMap<String, String> = t.plan_anchor_phase_by_kind.clone();
    for kind in t.plans_by_kind.keys() {
        anchor_by_kind
            .entry(kind.clone())
            .or_insert_with(|| format!("{kind}_plan"));
    }
    ...
    for (idx, ph) in t.phases.iter().enumerate() {
        ...
        for (kind, p) in t.plans_by_kind.iter() {
            if anchor_by_kind.get(kind).map(String::as_str) == Some(ph.as_str()) {
                render_plan_summary_line(&mut lines, kind.as_str(), p);
            }
        }
...
mod tests {
    ...
    fn render_plan_tree_includes_work_item_and_span() {
        ...
        let mut task = api::PlanTask::new(
            "t1".to_string(),
            api::PlanTaskStatus::InProgress,
            vec![ci],
        );
        task.task_kind = Some("cleanse".to_string());
        task.label = Some("ds1".to_string());
    ...
    fn render_plan_workgroups_compact_keeps_checklist_line_sticky_after_focus_moves() {
        ...
        let mut task1 = api::PlanTask::new(
            "t1".to_string(),
            api::PlanTaskStatus::InProgress,
            vec![ci.clone()],
        );
