...
use crate::qa::vector::global_lance_store::GlobalLanceDbStore;
...
pub async fn ensure_synced_once() {
	let _ = SYNC_ONCE.get_or_init(|| async {
		...
		match sync_from_repo_to_s3_and_embeddings().await {
			Ok(_) => (),
...
async fn load_global_manifest() -> GlobalManifest {
...
async fn save_global_manifest(m: &GlobalManifest) {
	let key = "dbt-examples/manifest.json";
	let _ = crate::helpers::s3::put_json(key, &serde_json::to_value(m).unwrap_or(Value::Null)).await;
}
...
pub async fn sync_from_repo_to_s3_and_embeddings() -> Result<(), String> {
	...
	for (project, files) in projects.iter() {
		...
		for (pathbuf, bytes) in files.iter() {
			...
			let _ = crate::helpers::s3::put_bytes(&key, bytes, ct).await;
		}
		if needs_reembed {
			// Build chunks
			...
			for (pathbuf, bytes) in files.iter() {
				...
				for (idx, part) in parts.iter().enumerate() {
					...
					let meta = serde_json::json!({
						"project": project,
						"path": r,
						"s3_uri": format!("s3://{}/dbt-examples/projects/{}/{}", bucket, project, r),
					});
					chunks.push(Chunk {
						id,
	...
	save_global_manifest(&g).await;
	info!("dbt_examples: sync complete (projects={})", g.projects.len());
	Ok(())
}

pub async fn search_examples(query: &str, k: usize) -> Result<Vec<ScoredChunk>, String> {
	let cfg = crate::llm::config_from_env();
	let llm = crate::llm::create_llm(&cfg);
