use skippr_plugin_data_source_google_ads::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-google-ads: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-google-ads",
        "GoogleAds",
        Some(capability),
        |start| {
            Box::pin(async move {
