...
impl OpenAICompatModel {
    pub fn new(cfg: LlmConfig) -> Self {
        ...
        let http_timeout_secs: u64 =
            crate::helpers::configuration::Config::getenv("LLM_HTTP_TIMEOUT_SECS", "120")
                .parse()
                .unwrap_or(120);
...
impl LargeLanguageModel for OpenAICompatModel {
    fn chat(
        ...
        if use_responses {
            ...
            if let Some(k) = self.cfg.api_key.as_ref() {
                ...
                match res {
                    Ok(r) => {
                        if r.status() == 429 && attempt < max_retries {
                            ...
                            let retry_after =
                                r.header("retry-after").and_then(|s| s.parse::<u64>().ok());
            ...
            if !(200..300).contains(&resp.status()) {
                ...
                tracing::debug!(
                    "LLM(responses) non-2xx status={} body:\n{}",
                    status,
                ...
                if status == 400 && (model.starts_with("gpt-5") || model.starts_with("o4")) {
                    ...
                    let mut req_cc = self
                        ...
                        .set("Content-Type", "application/json");
                    if let Some(k) = self.cfg.api_key.as_ref() {
                        req_cc = req_cc.set("Authorization", &format!("Bearer {}", k));
                    }
                    let cc_payload = serde_json::to_value(&cc_body).map_err(|e| e.to_string())?;
                    tracing::debug!(
                        "LLM(chat.completions) fallback request model='{}'\n{}",
                        cc_body.model,
                        pretty_val(&cc_payload)
                    );
                    ...
                    match res_cc {
                        Ok(rcc) if (200..300).contains(&rcc.status()) => {
                            ...
                            tracing::debug!(
                                "LLM(chat.completions) fallback response:\n{}",
                                pretty_json(&text)
                            );
                            let obj: OaiChatResp =
                        ...
                        Ok(rcc) => {
                            ...
                            tracing::debug!(
                                "LLM(chat.completions) fallback non-2xx={} body:\n{}",
                                status_cc,
                        ...
                        Err(ecc) => {
                            return Err(format!(
                                "http 400 (responses) and fallback chat error: {}; {}",
                                ecc, snippet
                            ));
                        }
                    }
            ...
            // Debug: pretty-print full prompt
            tracing::debug!(
                "LLM(chat.completions) request model='{}'\n{}",
                body.model,
                pretty_val(&payload)
            );
            let mut attempt = 0usize;
            ...
            if !(200..300).contains(&resp.status()) {
                ...
                let body_text = resp.into_string().unwrap_or_else(|_| String::new());
                tracing::debug!(
                    "LLM(chat.completions) non-2xx status={} body:\n{}",
                    status,
                    pretty_json(&body_text)
                );
    ...
    fn embed(&self, texts: &[String]) -> Result<Vec<Vec<f32>>, String> {
        ...
        let model = self
            ...
            .embed_model
            .clone()
            .ok_or_else(|| "missing embed_model".to_string())?;
        let url = format!("{}/v1/embeddings", base.trim_end_matches('/'));
        // Debug: pretty-print embedding inputs
        tracing::debug!(
            "LLM(embeddings) request model='{}' inputs:\n{}",
            model,
            pretty_json(&serde_json::to_string(texts).unwrap_or_else(|_| "[]".to_string()))
        );
        let body = OaiEmbReq {
            model,
            input: texts.to_vec(),
        ...
        if !(200..300).contains(&resp.status()) {
            ...
            let body_text = resp.into_string().unwrap_or_else(|_| String::new());
            tracing::debug!(
                "LLM(embeddings) non-2xx status={} body:\n{}",
                status,
                pretty_json(&body_text)
            );
            return Err(format!("http {}", status));
