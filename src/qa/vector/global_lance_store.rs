use crate::qa::vector::lance_store::{Chunk, ScoredChunk};
use serde_json::Value;

/// Global LanceDB store for DBT example embeddings.
/// URI layout: s3://{SKIPPR_S3_BUCKET}/dbt-examples/lancedb
...
impl GlobalLanceDbStore {
	...
	fn uri() -> String {
		...
		format!("s3://{}/dbt-examples/lancedb", bucket)
	}
	...
	pub async fn upsert(&self, items: &[Chunk]) -> Result<(), String> {
		...
		let namespaces = StringArray::from(items.iter().map(|c| c.namespace.clone()).collect::<Vec<_>>());
		...
		let batch = RecordBatch::try_new(schema, vec![
			Arc::new(ids), Arc::new(kinds), Arc::new(namespaces), Arc::new(fields), Arc::new(texts), Arc::new(epochs), Arc::new(fsl)
		]).map_err(|e| e.to_string())?;
		// Connect and upsert
		let uri = Self::uri();
		info!("LanceDB(Global): connecting uri='{}'", uri);
		...
		info!("LanceDB(Global): upserted rows={} dims={}", items.len(), dims);
		Ok(())
	}
		...
		while let Some(batch_res) = stream.next().await {
			...
			for r in 0..b.num_rows() {
				let s_val = |i: Option<usize>| -> String {
					i.map(|j| crate::sql::tui::value_to_string(b.column(j).as_ref(), r)).unwrap_or_default()
				};
				let id = s_val(id_i);
				let kind = s_val(kind_i);
				let namespace = s_val(ns_i);
				...
				out.push(ScoredChunk { item: Chunk { id, kind, namespace, field, text, vector: Vec::new(), meta: Value::Null, epoch }, score });
			}
		}
		Ok(out)
	}
