...
use crate::react::providers::VectorStore;
use std::sync::Arc;
...
pub async fn sync_pipeline(
    ...
    vector: Arc<dyn crate::react::providers::VectorStore>,
    pipeline: &str,
) -> Result<(), String> {
    ...
    for k in keys.iter() {
        ...
        if let Some(rest) = k.strip_prefix(&catalog_prefix) {
            ...
            if !stem.is_empty() {
                namespaces.push(percent_decode(stem));
            }
        }
    }
    namespaces.sort();
    namespaces.dedup();
    info!(
        "Embeddings: starting sync for pipeline='{}' (catalogs={})",
        pipeline,
        namespaces.len()
    );
    ...
    let mut items: Vec<crate::react::vector::lance_store::Chunk> = Vec::new();
    ...
    for ns in namespaces.iter() {
        ...
        if let Ok(val) = storage.get_json(&catalog_key).await {
            ...
            if let Some(fields) = val.get("fields").and_then(|x| x.as_array()) {
                for f in fields {
                    ...
                    items.push(crate::react::vector::lance_store::Chunk {
        ...
        {
            let base = keyspace.dbt_prefix(scope, pipeline).trim_end_matches('/').to_string();
    ...
    while p < items.len() {
        ...
        vector.upsert(pipeline, &items[p..q]).await?;
