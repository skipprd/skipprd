...
| Cloud tables mesh auth | n/a (DynamoDB Local) | Host injects GuestCredentialBroker (`CLOUD_SYSTEM_BROKER_CONFIG`). skipprd’s Cloud tables plugin POSTs CatalogJwt to that endpoint. MUST NOT `CLOUD_TABLES_ACCESS_TOKEN` / `CLOUD_ACCESS_TOKEN`. MUST NOT compile Cloud crates. |
