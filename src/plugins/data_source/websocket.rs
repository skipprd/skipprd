...
impl From<DataSourcePluginConfig> for DataSourceWebsocketPluginConfig {
    fn from(plugin_config: DataSourcePluginConfig) -> Self {
        match plugin_config {
            DataSourcePluginConfig::Websocket(config) => config,
            _ => panic!("Invalid plugin type for Websocket"),
        }
    }
}
