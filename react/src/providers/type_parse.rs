...
///
/// Today this targets Athena/Glue style types like:
...
pub struct FlattenedField {
    /// Dot-path, e.g. `context.session.id`
    pub path: String,
    /// Engine-native type string for this leaf
    pub data_type: String,
    /// Optional SQL expression to reference this leaf (best-effort, may be None)
    pub expr: Option<String>,
    /// True if this leaf is complex (struct/row/array/map)
    pub is_complex: bool,
}
...
fn parse_type_into(out: &mut Vec<FlattenedField>, stack: &mut Vec<String>, ty: &str) {
    ...
    if let Some(inner) = strip_wrapped(t, "struct<", '>') {
        for (name, child_ty) in parse_struct_fields(inner).into_iter() {
            stack.push(name);
    ...
    if let Some(inner) = strip_wrapped(t, "row(", ')') {
        for (name, child_ty) in parse_row_fields(inner).into_iter() {
            stack.push(name);
            parse_type_into(out, stack, &child_ty);
            stack.pop();
        }
...
fn push_leaf(out: &mut Vec<FlattenedField>, stack: &Vec<String>, ty: &str, is_complex: bool) {
    let path = stack.join(".");
    let expr = build_safe_deref_expr(stack);
    out.push(FlattenedField {
        path,
        data_type: ty.to_string(),
        expr,
