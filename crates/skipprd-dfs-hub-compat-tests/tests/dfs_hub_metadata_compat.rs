...
use skippr_plugin_data_source_dataforseo_seo_opportunities::streams::{
    ...
    NAMESPACE_OPPORTUNITY_SCORE_DAILY, NAMESPACE_SEED_KEYWORD_DAILY, NAMESPACE_SITE_RUN_DAILY,
};
...
use skipprd::ingest::fast_ingest::{
    create_default_nested_message, fast_path_ingest, DEFAULT_NESTED_MESSAGE,
};
use skipprd::ingest_work::{storage_namespace, Ingest};
...
fn metadata_path() -> PathBuf {
    std::env::var("SKIPPR_DFS_HUB_METADATA_PATH").map_or_else(
        |_| {
            repo_root().join("../../upfoundry/console-web/skippr/pipeline-metadata/dataforseo_seo_opportunities.json")
        },
        PathBuf::from,
    )
}
...
fn load_pipeline_metadata() -> PipelineMetadata {
    ...
    serde_json::from_str(&raw).unwrap_or_else(|e| panic!("parse pipeline metadata: {e}"))
}
...
impl SourceSyncContext for RecordingSyncContext {
    ...
    fn store_checkpoint(&self, _key: &str, _envelope: &CheckpointEnvelope) -> Result<(), String> {
        Ok(())
    }
...
fn install_namespace_template(
    ...
    let ns_meta = pipeline
        ...
        .clone();
    let fields = ns_meta.compat_fields().clone();
...
async fn dfs_hub_fixture_rows_match_console_metadata_fast_path() {
    let fixture_dir = repo_root().join("plugins/data_source/dataforseo_seo_opportunities/fixtures");
    ...
    for plugin_ns in namespaces {
        ...
        for (idx, row) in rows.into_iter().enumerate() {
            ...
            match fast_path_ingest(&row, &fields, &storage_ns, false) {
                Ok(_) => {}
                Err(err) => failures.push(format!("ns={storage_ns} row={idx} err={err} row={row}")),
            }
        }
    }
...
fn dfs_hub_metadata_covers_plugin_emitted_fields() {
    ...
    for plugin_ns in plugin_namespaces {
        ...
        let meta_fields: HashSet<_> = pipeline
            .metadata
            .get(&storage_ns)
            .map(|m| m.compat_fields().keys().cloned().collect())
            .unwrap_or_default();
