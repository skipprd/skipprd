...
pub const CURATED_STREAMS: &[MetaStreamDef] = &[
    MetaStreamDef {
        namespace: "linkedin_ads.account_daily",
        level: InsightsLevel::Account,
        placement_breakdown: false,
    },
    MetaStreamDef {
        namespace: "linkedin_ads.campaign_daily",
        level: InsightsLevel::Campaign,
        placement_breakdown: false,
    },
    MetaStreamDef {
        namespace: "linkedin_ads.adset_daily",
        level: InsightsLevel::Adset,
        placement_breakdown: false,
    },
    MetaStreamDef {
        namespace: "linkedin_ads.ad_daily",
        level: InsightsLevel::Ad,
        placement_breakdown: false,
    },
    MetaStreamDef {
        namespace: "linkedin_ads.campaign_placement_daily",
        level: InsightsLevel::Campaign,
        placement_breakdown: true,
    },
];

const MINIMAL_NAMESPACES: &[&str] = &["linkedin_ads.account_daily"];

const STANDARD_NAMESPACES: &[&str] = &[
    "linkedin_ads.account_daily",
    "linkedin_ads.campaign_daily",
    "linkedin_ads.adset_daily",
];
...
mod tests {
    ...
    fn minimal_profile_is_account_only() {
        ...
        assert_eq!(streams[0].namespace, "linkedin_ads.account_daily");
    }
    ...
    fn resolve_streams_explicit_override_filters_catalog() {
        let selected = resolve_streams(
            StreamProfile::Full,
            Some(vec!["linkedin_ads.ad_daily".into()]),
        );
        assert_eq!(selected.len(), 1);
        assert_eq!(selected[0].namespace, "linkedin_ads.ad_daily");
    }
    ...
    fn placement_stream_uses_campaign_level_with_breakdown() {
        let placement = CURATED_STREAMS
            .iter()
            .find(|s| s.namespace == "linkedin_ads.campaign_placement_daily")
            .unwrap();
