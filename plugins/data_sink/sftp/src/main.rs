...
use skippr_plugin_runtime_link::runtime_sink_link::sftp::{DataSinkSftpPlugin, DataSinkSftpPluginConfig};
...
async fn run() -> io::Result<()> {
    ...
    let handshake = match read_host_frame(&mut stdin).await? {
        ...
        other => {
            ...
            return Err(io::Error::other(
                "runtime sftp sink did not receive a handshake",
            ));
        }
    };
    ...
    let response = HandshakeResponse {
        ...
        plugin_name: "Sftp".to_string(),
        source_capability: None,
        sink_capability: cdc::sink_capabilities::by_name("Sftp").map(Into::into),
...
{
    let mut primary_plugin: Option<DataSinkSftpPlugin> = None;
    let mut deadletter_plugin: Option<DataSinkSftpPlugin> = None;
...
async fn run_sink_request(
    ...
    primary_plugin: &mut Option<DataSinkSftpPlugin>,
    deadletter_plugin: &mut Option<DataSinkSftpPlugin>,
) -> io::Result<()> {
    request
        .config
        .0
    ...
    if plugin_slot.is_none() {
        ...
        *plugin_slot = Some(
            DataSinkSftpPlugin::new_with_config(buffer_name_for_binding(request.binding), config)
                .await,
        );
    }
