use skippr_plugin_data_source_adroll_ads::*;
...
skippr_runtime_sdk::runtime_main!(async {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-adroll-ads: {}", err);
        std::process::exit(1);
    }
});
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-adroll-ads",
        "AdRollAds",
        Some(capability),
        |start| {
            Box::pin(async move {
