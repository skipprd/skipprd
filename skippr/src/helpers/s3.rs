            ...
            .await
        {
            ...
            Err(e) => {
                ...
                if attempt >= max_attempts {
                    println!(
                        "Failed to upload metrics to S3 after {} attempts: {:?}",
                        attempt, e
                    );
                    return Err(e.into());
                }
    ...
    loop {
        ...
        match req.send().await {
            ...
            Err(e) => {
                ...
                if attempt >= max_attempts {
                    println!(
                        "Failed to upload bytes to S3 after {} attempts: {:?}",
                        attempt, e
                    );
                    return Err(e.into());
                }
...
pub async fn delete_prefix(prefix: &str) -> Result<usize, String> {
    ...
    if !prefix.contains('/') {
        return Err("Refusing to delete top-level prefix without '/'".to_string());
    }
    ...
    loop {
        ...
        let resp = req
            .send()
            .await
            .map_err(|e| format!("list_objects_v2 error: {:?}", e))?;
        ...
        if keys.is_empty() {
            ...
            for k in keys.iter() {
                if let Ok(obj) = ObjectIdentifier::builder().key(k).build() {
                    batch.push(obj);
                }
                if batch.len() == 1000 {
                    ...
                    client
                        ...
                        .send()
                        .await
                        .map_err(|e| format!("delete_objects error: {:?}", e))?;
                    total += 1000;
                    batch = Vec::new();
...
pub async fn delete_prefix_in_bucket(bucket: &str, prefix: &str) -> Result<usize, String> {
    ...
    // Safety checks
    if bucket.is_empty() {
        return Err("Bucket required".to_string());
    }
    if prefix.is_empty() || prefix == "/" || prefix == "." || prefix == ".." {
        return Err("Refusing to delete unsafe or empty prefix".to_string());
    }
                ...
                for obj in contents {
                    if let Some(k) = obj.key() {
                        if k.ends_with(".parquet") {
                            out.push((
                                k.to_string(),
                                obj.last_modified().map(|t| t.secs()).unwrap_or_default(),
                            ));
                        }
                    }
                }
