...
- dbt_files(args:{op:"list"|"get"|"get_json"|"manifest_find"|"patch", prefix?:string, path?:string, patch_text?:string, replace_file?:{path:string,new_text:string,expected_sha256?:string}|[{...}], replace_range?:{path:string,start_line:int,end_line:int,new_text:string,expected_sha256?:string}|[{...}], replace_list?:{path:string,edits:[{start_line:int,end_line:int,new_text:string}],expected_sha256?:string}|[{...}], pointer?:string, limit?:int, preview_diff?:bool, max_chars?:int})
...
- For `dbt_files op=patch`, provide EXACTLY ONE of: patch_text OR replace_file OR replace_range OR replace_list. The tool will compute and return `applied_patch_text` (canonical git-style diff) for audit.
