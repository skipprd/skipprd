use skippr_plugin_data_source_hubspot_crm::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-hubspot-crm: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-hubspot-crm",
        "HubspotCrm",
        Some(capability),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("HubspotCrm")
                    .map_err(std::io::Error::other)?;
                let cfg: DataSourceHubspotCrmPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourceHubspotCrmPlugin::new(cfg)?;
