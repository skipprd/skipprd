...
use crate::discover::metadata_apply::{apply_namespace_field_rows, MetadataFieldRow};
...
pub struct MetadataApplyArgs {
    ...
    /// Inline schema JSON (same shape as `--schema` file).
...
fn emit_metadata_json<T: Serialize>(value: &T) {
    println!(
        "{}",
...
pub fn pipeline_for_action(action: &MetadataAction) -> &str {
    match action {
        MetadataAction::Show(args) => args.pipeline.as_str(),
        MetadataAction::Apply(args) => args.pipeline.as_str(),
    }
}
