...
impl DataEngineerSuite {
    ...
    fn build_tools(agent_type: &str, sctx: &SuiteCtx) -> Result<ToolRegistry, String> {
        ...
        impl crate::tools::Tool for GuardedDbtValidateTool {
            fn name(&self) -> &'static str { "dbt_validate" }
            async fn call(&self, args: serde_json::Value, ctx: &crate::agent::AgentCtx) -> Result<serde_json::Value, String> {
        ...
        #[async_trait::async_trait]
        impl crate::tools::Tool for GuardedSqlRunTool {
        ...
        #[async_trait::async_trait]
        impl crate::tools::Tool for GuardedApproveAndSaveArtifactTool {
        ...
        #[async_trait::async_trait]
        impl crate::tools::Tool for GuardedApproveAndSaveArtifactBatchTool {
        ...
        #[async_trait::async_trait]
        impl crate::tools::Tool for GuardedStagingModelTool {
        ...
        #[async_trait::async_trait]
        impl crate::tools::Tool for GuardedDbtFilesTool {
        ...
        match agent_type {
            ...
            "review" => {
                ...
                #[async_trait::async_trait]
                impl crate::tools::Tool for ReadOnlyDbtFilesTool {
                    fn name(&self) -> &'static str { "dbt_files" }
                    async fn call(&self, args: serde_json::Value, ctx: &crate::agent::AgentCtx) -> Result<serde_json::Value, String> {
    ...
    async fn run_review(thread_id: &str, question: &str, sctx: &SuiteCtx) -> Result<Vec<FlowFrame>, String> {
        ...
        let actx = AgentCtx {
            ...
            policy: std::sync::Arc::new(crate::agent::DefaultPolicy),
            llm: sctx.llm.clone(),
            storage: sctx.storage.clone(),
            scope: sctx.scope.clone(),
            keyspace: sctx.keyspace.clone(),
...
mod tests {
    ...
    impl crate::providers::QueryProvider for MockQuery {
        async fn query(&self, _sql: &str) -> Result<crate::providers::QueryResult, String> {
            Ok(crate::providers::QueryResult { header: vec![], rows: vec![], meta: None })
        }
