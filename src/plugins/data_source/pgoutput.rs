...
/// A single decoded tuple row — each element is `None` (SQL NULL) or `Some(text)`.
...
fn parse_tuple(cur: &mut Cursor<'_>) -> Option<TupleData> {
    ...
    for _ in 0..n_cols {
        ...
        match tag {
            ...
            // 'u' = unchanged TOAST column — treat as NULL for our purposes
            _ => row.push(None),
        }
    }
    Some(row)
...
fn parse_update(data: &[u8]) -> Option<PgOutputMessage> {
    ...
    let old_row = if tag == b'K' || tag == b'O' {
        ...
        let _new_tag = cur.read_u8()?; // consume the 'N' tag
        Some(tuple)
    } else {
        // tag is already 'N'
        None
...
mod tests {
    ...
    fn build_relation_msg() -> Vec<u8> {
        ...
        buf.push(b'R');
        buf.extend_from_slice(&1234u32.to_be_bytes()); // oid
        ...
        buf.extend_from_slice(&(-1i32 as u32).to_be_bytes()); // type modifier
                                                              // col 2
        buf.push(0);
    ...
    fn test_parse_insert() {
        ...
        buf.push(b'N');
        buf.extend_from_slice(&2u16.to_be_bytes()); // 2 cols
        buf.push(b't');
        let val = b"42";
