...
mod json_benchmarks {
    ...
    const CONCATENATED_JSON: &str = r#"{"name":"first","value":1}{"name":"second","value":2}{"name":"third","value":3}"#;
    ...
    fn benchmark_comparison() {
        ...
        let opt_time = benchmark("Optimized", 10000, || {
            parser.parse(STANDARD_JSON)
        });
        
        let improvement = (orig_time.as_nanos() as f64 / opt_time.as_nanos() as f64) * 100.0 - 100.0;
        ...
        let opt_time = benchmark("Optimized", 5000, || {
            parser.parse(COMPLEX_NESTED_JSON)
        });
        ...
        let opt_time = benchmark("Optimized", 1000, || {
            parser.parse(LARGE_NESTED_JSON)
        });
