use crate::vector::lance_store::{Chunk, ScoredChunk};
	...
	pub async fn query(&self, query_vec: &[f32], k: usize) -> Result<Vec<ScoredChunk>, String> {
		...
		while let Some(batch_res) = stream.next().await {
			...
			for r in 0..b.num_rows() {
				...
				out.push(ScoredChunk { item: Chunk { id, kind, dataset_id, field, text, vector: Vec::new(), meta: Value::Null, epoch }, score });
			}
		}
		Ok(out)
	}
