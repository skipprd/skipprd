use skippr_plugin_data_source_upfoundry_backlinks::*;
...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-upfoundry-backlinks: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    let capability = RuntimeSourceCapabilityDescriptor {
        name: "UpfoundryBacklinks".to_string(),
    ...
    run_append_data_source_main(
        "skippr-plugin-data-source-upfoundry-backlinks",
        "UpfoundryBacklinks",
        Some(capability),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("UpfoundryBacklinks")
                    .map_err(std::io::Error::other)?;
                let cfg: UpfoundryBacklinksConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = UpfoundryBacklinksPlugin::new(cfg)?;
