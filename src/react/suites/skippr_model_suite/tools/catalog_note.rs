...
impl Tool for CatalogNoteTool {
	...
	async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
		...
		let dataset_id = args.get("dataset_id").and_then(|x| x.as_str()).unwrap_or("").trim().to_string();
		if dataset_id.is_empty() { return Err("dataset_id required".to_string()); }
		...
		// Write back atomically
			ctx.storage.put_json(&catalog_key, &catalog).await.map_err(|e| format!("put_json: {}", e))?;
		...
		if !vec1.is_empty() {
			...
			let chunk = crate::react::vector::lance_store::Chunk {
				id,
				kind: "doc".to_string(),
				namespace: dataset_id.clone(),
			...
			let _ = vector.upsert(&ctx.scope.project_id, &[chunk]).await;
		}
		let est_tokens = ((chat_in_chars + chat_out_chars) as f32 / 4.0).round() as i64;
