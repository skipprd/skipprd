...
use skippr_plugin_runtime_link::runtime_sink_link::amqp::{DataSinkAmqpPlugin, DataSinkAmqpPluginConfig};
...
async fn run() -> io::Result<()> {
    ...
    let handshake = match read_host_frame(&mut stdin).await? {
        ...
        other => {
            ...
            return Err(io::Error::other(
                "runtime amqp sink did not receive a handshake",
            ));
        }
    };
    ...
    let response = HandshakeResponse {
        ...
        plugin_name: "Amqp".to_string(),
        source_capability: None,
        sink_capability: cdc::sink_capabilities::by_name("Amqp").map(Into::into),
...
{
    let mut primary_plugin: Option<DataSinkAmqpPlugin> = None;
    let mut deadletter_plugin: Option<DataSinkAmqpPlugin> = None;
...
async fn run_sink_request(
    ...
    primary_plugin: &mut Option<DataSinkAmqpPlugin>,
    deadletter_plugin: &mut Option<DataSinkAmqpPlugin>,
) -> io::Result<()> {
    request
        .config
        .0
    ...
    if plugin_slot.is_none() {
        ...
        *plugin_slot = Some(
            DataSinkAmqpPlugin::new_with_config(buffer_name_for_binding(request.binding), config)
                .await,
        );
    }
