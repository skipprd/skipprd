...
async fn main() {
    if let Err(err) = run().await {
        eprintln!("skippr-plugin-data-source-statsd: {}", err);
        std::process::exit(1);
    }
}
...
async fn run() -> std::io::Result<()> {
    run_append_data_source_main(
        "skippr-plugin-data-source-statsd",
        "Statsd",
        cdc::source_capabilities::by_name("Statsd").map(Into::into),
        |start| {
            Box::pin(async move {
                start
                    .config
                    .0
                    .expect_plugin("Statsd")
                    .map_err(std::io::Error::other)?;
                let cfg: DataSourceStatsdPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourceStatsdPlugin::with_runtime_config(cfg);
