...
| `--output json` / `--output text` | `skippr sync --output json` | Structured output for programmatic parsing |
| `--once` flag | `skippr sync --once` | Single sync pass (bounded execution) |
| Local Metadata Persistence | `SKIPPR_STORAGE_MODE=local` | Read/write metadata and stats to local disk |
| Discover Mode Enhancements | `skippr discover --output json` | Schema-only mode with structured output, no output plugin |
...
3. Execute: `skippr query --sql "LOAD SCHEMA '/tmp/schema.json' INTO el_mssql" --plain`
...
```bash
skippr sync --pipeline el_mssql --output json   # JSON lines to stdout
skippr sync --pipeline el_mssql --output text   # Plain text to stdout
skippr sync --pipeline el_mssql --output progress  # Default interactive spinner
```
...
```bash
skippr sync --pipeline el_mssql --once
```
...
`Config::get_metadata()`, `Config::set_metadata()`, and related functions were S3-only. skippr-dbt runs skippr in local disk mode with no S3 bucket configured.
...
`skippr discover` no longer initializes or syncs to any output plugin. It uses a no-op output internally. The discover flow is now:
...
This is how skippr-dbt retrieves the discovered source schemas to feed into the LLM mapping phase after running `skippr discover`.
...
skippr query --sql "LOAD SCHEMA '/tmp/schema.json' INTO el_mssql" --plain
...
```bash
skippr sync --pipeline el_mssql --once --output json 2>/dev/null | while read -r line; do
