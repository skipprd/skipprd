...
impl Tool for VectUpsertTool {
    ...
    async fn call(&self, args: Value, ctx: &AgentCtx) -> Result<Value, String> {
        ...
        let texts: Vec<String> = arr
            .iter()
            .map(|v| v.get("text").and_then(|x| x.as_str()).unwrap_or("").to_string())
            .collect();
        ...
        for (i, v) in arr.iter().enumerate() {
            ...
            items.push(VectorChunk { id, kind, dataset_id, field, text, vector: vecs.get(i).cloned().unwrap_or_default(), meta, epoch });
        }
