...
const GAADS_SCOPE: &str = "https://www.googleapis.com/auth/webmasters.readonly";
...
pub fn gaads_webmasters_scope() -> &'static str {
    GAADS_SCOPE
}
...
pub async fn authorization_header(
    ...
    Err(std::io::Error::new(
        ...
        "Google Search Console requires access_token, OAuth refresh credentials, \
         ...
         SKIPPR_GOOGLE_ADS_FIXTURE_DIR is set)",
    ))
}
...
pub async fn search_analytics_query_page(
    ...
    if let Ok(fixture_dir) = std::env::var("SKIPPR_GOOGLE_ADS_FIXTURE_DIR") {
    ...
    loop {
        ...
        match RetryableHttpClient::classify_status(status, retry_after)
        {
            ...
            skippr_plugin_shared_api_source::RetryDecision::RetryAfter(delay) => {
                ...
                if attempt >= http.config.max_attempts {
                    return Err(std::io::Error::other(format!(
                        "GAADS request failed after {attempt}/{} attempts: HTTP {status} {url}",
                ...
                warn!(
                    ...
                    %status,
                    %url,
                    "GAADS rate limited or transient error; backing off before retry"
                );
            ...
            skippr_plugin_shared_api_source::RetryDecision::GiveUp => {
                ...
                return Err(std::io::Error::other(format!(
                    "GAADS request failed: HTTP {status} {text}"
                )));
            }
        }
    }
...
mod tests {
    ...
    fn optional_stream_error_detects_400() {
        let err = std::io::Error::other(
            "GAADS request failed: HTTP 400 Bad Request INVALID_ARGUMENT: dimension searchAppearance",
        );
    ...
    fn forbidden_site_error_detects_403() {
        let err = std::io::Error::other("GAADS request failed: HTTP 403 Forbidden");
    ...
    fn optional_stream_skipped_on_400() {
        let err = std::io::Error::other(
            "GAADS request failed: HTTP 400 dimension searchAppearance not supported",
        );
        assert!(is_optional_stream_error(&err));
        assert!(!is_forbidden_site_error(&err));
    }
