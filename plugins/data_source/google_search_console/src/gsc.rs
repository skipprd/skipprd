...
const GSC_HISTORY_MONTHS: i64 = 16;
...
pub struct DataSourceGoogleSearchConsolePluginConfig {
    pub site_url: String,
    #[serde(default)]
    #[skippr(secret)]
...
impl DataSourceGoogleSearchConsolePluginConfig {
    pub fn validate(&self) -> Result<(), std::io::Error> {
        ...
        let earliest = Utc::now().date_naive() - Duration::days(GSC_HISTORY_MONTHS * 30);
        if start < earliest {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "start_date {start} is older than ~{GSC_HISTORY_MONTHS} months of Search Console history"
                ),
            ));
        }
        Ok(())
...
pub struct DataSourceGoogleSearchConsolePlugin {
    config: DataSourceGoogleSearchConsolePluginConfig,
    http: RetryableHttpClient,
    oauth: Option<OAuth2RefreshTokenAuth>,
}

impl DataSourceGoogleSearchConsolePlugin {
    pub fn new(config: DataSourceGoogleSearchConsolePluginConfig) -> Result<Self, std::io::Error> {
        config.validate()?;
    ...
    async fn auth_header(&self) -> Result<String, std::io::Error> {
        if std::env::var("SKIPPR_GOOGLE_SEARCH_CONSOLE_FIXTURE_DIR")
            .ok()
            ...
            .await
            {
                ...
                Err(err) if is_forbidden_site_error(&err) => {
                    return Err(std::io::Error::new(
                        ...
                        format!(
                            "GSC permission denied for site_url '{}': {err}. Verify the OAuth \
    ...
    async fn sync_sitemap_snapshot(
        ...
        self.submit_rows_for_date(
            ctx.as_ref(),
            "google_search_console.sitemap_daily",
            partition_date,
            rows,
        )?;
        Ok(count)
    ...
    async fn emit_site_run_daily(
        ...
        self.submit_rows_for_date(
            ctx.as_ref(),
            "google_search_console.site_run_daily",
            partition_date,
            vec![row],
        )
    }
...
#[async_trait]
impl DataSource for DataSourceGoogleSearchConsolePlugin {
    ...
    fn source_namespace_contracts(&self) -> Vec<SourceNamespaceContract> {
        ...
        for contract in &contracts {
            contract
                .validate()
                .expect("invalid GSC namespace contract configuration");
        }
        contracts
    }
    ...
    async fn sync(&mut self, ctx: Arc<dyn SourceSyncContext>) -> Result<(), std::io::Error> {
        ...
        let start_date = if discover {
            ...
            info!(
                ...
                "GSC discover: sampling recent days only; full historical sync runs on skippr sync"
            );
            sample_start
        } else {
            configured_start
        ...
        if end_date < start_date {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!("GSC end date {end_date} is before start date {start_date}"),
            ));
        }

        let auth_header = self.auth_header().await?;
        if !discover
            && std::env::var("SKIPPR_GOOGLE_SEARCH_CONSOLE_FIXTURE_DIR")
                .ok()
                .filter(|v| !v.trim().is_empty())
                .is_none()
        {
        ...
        if discover {
            info!(
                ...
                "GSC discover: sampling minimal stream profile for API calls"
            );
        }
...
mod tests {
    ...
    fn discover_mode_uses_minimal_streams_only() {
        let plugin = DataSourceGoogleSearchConsolePlugin::new(test_config()).unwrap();
