...
impl From<DataSourcePluginConfig> for DataSourceEventbridgePluginConfig {
    fn from(plugin_config: DataSourcePluginConfig) -> Self {
        match plugin_config {
            DataSourcePluginConfig::Eventbridge(config) => config,
            _ => panic!("Invalid plugin type for Eventbridge"),
        }
    }
}
