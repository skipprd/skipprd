...
use skippr_plugin_data_source_postgres::*;
...
async fn run() -> std::io::Result<()> {
    run_append_data_source_main(
        ...
        "Postgres",
        cdc::source_capabilities::by_name("Postgres").map(Into::into),
        |start| {
            Box::pin(async move {
                start
                    .config
                ...
                let cfg: DataSourcePostgresPluginConfig =
                    start.config.0.decode().map_err(std::io::Error::other)?;
                let plugin = DataSourcePostgresPlugin::with_runtime_config(cfg);
